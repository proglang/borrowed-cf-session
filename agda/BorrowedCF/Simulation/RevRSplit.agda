module BorrowedCF.Simulation.RevRSplit where
...
open import BorrowedCF.Simulation.ReverseInv using (νσ; ⊗-inj; νσ-VSub)
open import BorrowedCF.Simulation.RevLSplit using (fin-split)
open import BorrowedCF.Simulation.RevAdmin using (_≈_; ≋⇒≈)
open import BorrowedCF.Simulation.TranslationProperties using (≡→≋)
open import BorrowedCF.Simulation.InvFrame using (fn-rsplit-dom)
...
-- rsplit-arg-chan : the argument of a well-typed `rsplit s` application is typed
--   at a session type ⟨ s ; s′ ⟩ (mirror of lsplit-arg-chan).
...
module _ {m n : ℕ} (σ : m →ₛ n) (Vσ : VSub σ) (q b₁' b₂ : ℕ) where
  ...
  rsplit-recon :
    ...
    → Γ ; γ ⊢ₚ T.ν C₁ Bg (T.⟪ F₀ [ K (`rsplit s) ·¹ (` atkU) ]* ⟫ T.∥ P₁t)
    → F ≡ frame*-⋯ F₀ νσ0 Vνσ0
    → ((e₁ ⊗ (` 0F)) ⊗ e₂) ≡ (` atkU) ⋯ νσ0
    → P₁ ≡ U[ P₁t ] νσ0
    → U.ν (U.φ U.drop
            (U.⟪ (F ⋯ᶠ* weakenᵣ)
             ...
             U.∥ (P₁ U.⋯ₚ weakenᵣ)))
      ≡ U[ T.ν C₁ᴿ Bg (T.⟪ F₀ ⋯ᶠ* swk [ (` injG0) ⊗ (` injG1) ]* ⟫ T.∥ (P₁t T.⋯ₚ swk)) ] σ
  ...
  ... | _ , ρ⁻ , skipH , E₀ , Eeq , P₀ , Peq =
    cong U.ν (cong₂ U.φ φ-eq (cong₂ U._∥_ threadEq restEq))
    where
      φ-eq : U.drop ≡ ϕ[ q + 1 ]
      ...
      restEq : P₁ U.⋯ₚ weakenᵣ ≡ U[ P₁t T.⋯ₚ swk ] Σᴿ
      restEq =
          cong (U._⋯ₚ weakenᵣ) Resteq
        ■ U-σ⋯ P₁t
      ...
      threadEq : U.⟪ (F ⋯ᶠ* weakenᵣ)
                        [ ((wk e₁ ⊗ (` 1F)) ⊗ (` 0F)) ⊗ (((` 0F) ⊗ (` 1F)) ⊗ wk e₂) ]* ⟫
               ≡ U[ T.⟪ F₀ ⋯ᶠ* swk [ (` injG0) ⊗ (` injG1) ]* ⟫ ] Σᴿ
      threadEq = cong U.⟪_⟫
  ...
  rsplit-go Γ-S {s = s} b₁ refl z toℕz≡q {F₀ = F₀} {P₁t = P₁t} {argᴸ = argᴸ} argᴸ≡
    ...
    where
      P′ : T.Proc m
      P′ = T.ν C₁ᴿ Bg (T.⟪ F₀ ⋯ᶠ* swk [ (` injG0) ⊗ (` injG1) ]* ⟫ T.∥ (P₁t T.⋯ₚ swk))
      ...
      ⊢P' : _ ; _ ⊢ₚ T.ν C₁ Bg (T.⟪ F₀ [ K (`rsplit s) ·¹ (` atkU) ]* ⟫ T.∥ P₁t)
      ⊢P' = subst (λ v → _ ; _ ⊢ₚ T.ν C₁ Bg (T.⟪ F₀ [ K (`rsplit s) ·¹ v ]* ⟫ T.∥ P₁t))
              argᴸ≡atkU ⊢P

      argeq' : ((e₁ ⊗ (` 0F)) ⊗ e₂) ≡ (` atkU) ⋯ νσ0
      argeq' = argeq ■ cong (λ v → v ⋯ νσ0) argᴸ≡atkU

      recon : U.ν (U.φ U.drop
                    (U.⟪ (F ⋯ᶠ* weakenᵣ)
                           [ ((wk e₁ ⊗ (` 1F)) ⊗ (` 0F)) ⊗ (((` 0F) ⊗ (` 1F)) ⊗ wk e₂) ]* ⟫
                     U.∥ (P₁ U.⋯ₚ weakenᵣ)))
              ≡ U[ P′ ] σ
      ...
      stepAtk : Star TR._─→ₚ_
                  (T.ν C₁ Bg (T.⟪ F₀ [ K (`rsplit s) ·¹ (` atkU) ]* ⟫ T.∥ P₁t)) P′
      stepAtk = TR.R-RSplit {B₁ = []} {B₂ = []} {B = Bg} {q = q} {b₁ = b₁'} {s = s} {P = P₁t} {E = F₀} ◅ ε

      step : Star TR._─→ₚ_
               (T.ν C₁ Bg (T.⟪ F₀ [ K (`rsplit s) ·¹ argᴸ ]* ⟫ T.∥ P₁t)) P′
      step = subst (λ v → Star TR._─→ₚ_
                     (T.ν C₁ Bg (T.⟪ F₀ [ K (`rsplit s) ·¹ v ]* ⟫ T.∥ P₁t)) P′)
