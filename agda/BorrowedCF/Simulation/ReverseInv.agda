module BorrowedCF.Simulation.ReverseInv where
...
open import BorrowedCF.Simulation.Frames using (frame*-⋯; frame-plug*; ++ₛ-VSub; weaken-VSub)
open import BorrowedCF.Simulation.TranslationProperties using (≡→≋; Ub-V)
...
new-arg-notVar : ∀ {m} {Γ : Ctx m} {γ} {x : 𝔽 m} {s : 𝕊 0} {d} {T ϵ}
  → ChanCx Γ → Γ ; γ ⊢ K (`new s) ·⟨ d ⟩ (` x) ∶ T ∣ ϵ → ⊥
...
var-app-absurd : ∀ {m} {Γ : Ctx m} {γ} {x : 𝔽 m} {e₂ d T ϵ}
  → ChanCx Γ → Γ ; γ ⊢ (` x) ·⟨ d ⟩ e₂ ∶ T ∣ ϵ → ⊥
...
value-step V (E-Ctx (app₁ _ _ _)  red)    with V
... | ()
value-step V (E-Ctx (app₂ _ _ _)  red)    with V
... | ()
...
⋯→-app {e₁ = e₁} {e₂ = e₂} {e′ = e′} {d = d} step = go _ step refl
  where
    ...
    go _ (E-Ctx (app₁ e₃ d′ V?) inner) refl = inj₂ (inj₁ (_ , inner , refl , V?))
    go _ (E-Ctx (app₂ v d′ V?) inner) refl = inj₂ (inj₂ (V? , _ , inner , refl))
...
headApp : (σ : m →ₛ n) → (e : Tm m) {d : Dir} {a b : Tm n} → (e ⋯ σ) ≡ a ·⟨ d ⟩ b
  → (Σ[ x ∈ 𝔽 m ] e ≡ ` x)
  ⊎ (Σ[ a₀ ∈ Tm m ] Σ[ b₀ ∈ Tm m ] (e ≡ a₀ ·⟨ d ⟩ b₀) × (a ≡ a₀ ⋯ σ) × (b ≡ b₀ ⋯ σ))
headApp σ (` x)          eq   = inj₁ (x , refl)
headApp σ (a₀ ·⟨ d ⟩ b₀) refl = inj₂ (a₀ , b₀ , refl , refl , refl)
...
⋯→-reflect Γ-S (e₁ ·⟨ d ⟩ e₂) ⊢e σ Vσ step | inj₂ (inj₁ (e₁″ , inner , refl , V?))
  with α₁ , α₂ , (_ , _ , ⊢e₁) , (_ , _ , ⊢e₂) , _ ← IF.inv-app ⊢e
  with e₁′ , s₁ , refl ← ⋯→-reflect Γ-S e₁ ⊢e₁ σ Vσ inner
  = e₁′ ·⟨ d ⟩ e₂ , E-Ctx (app₁ e₂ d (λ p → value-⋯⁻¹ σ Vσ e₂ (V? p))) s₁ , refl
⋯→-reflect Γ-S (e₁ ·⟨ d ⟩ e₂) ⊢e σ Vσ step | inj₂ (inj₂ (V? , e₂″ , inner , refl))
  with α₁ , α₂ , (_ , _ , ⊢e₁) , (_ , _ , ⊢e₂) , _ ← IF.inv-app ⊢e
  with e₂′ , s₂ , refl ← ⋯→-reflect Γ-S e₂ ⊢e₂ σ Vσ inner
  = e₁ ·⟨ d ⟩ e₂′ , E-Ctx (app₂ e₁ d (λ p → value-⋯⁻¹ σ Vσ e₁ (V? p))) s₂ , refl
...
plugApp-not-value : ∀ {n} (F : Frame* n) {d} {a b : Tm n} → ¬ Value (F [ a ·⟨ d ⟩ b ]*)
plugApp-not-value [] ()
plugApp-not-value (app₁ _ _ _ ∷ F*) ()
plugApp-not-value (app₂ _ _ _ ∷ F*) ()
...
-- Congruences for the application frames: the value-witness component is
-- proof-irrelevant (funext + Value-irr), so an equality of the held operand
-- suffices to equate the frames regardless of the (dependent) V? functions.
...
app₂-cong : {e₁ e₂ : Tm n} {d : Dir}
            {V?₁ : d ≡ 𝟙 Sum.⊎ d ≡ R → Value e₁} {V?₂ : d ≡ 𝟙 Sum.⊎ d ≡ R → Value e₂}
          → e₁ ≡ e₂ → app₂ e₁ d V?₁ ≡ app₂ e₂ d V?₂
app₂-cong refl = cong (app₂ _ _) (funext (λ x → Value-irr))
...
frameApp-reflect Γ-S e₀ ⊢e σ Vσ c (app₁ e₃ d V? ∷ F*) eq
  with headApp σ e₀ eq
... | inj₁ (x , refl) = ⊥-elim (plugApp-not-value (app₁ e₃ d V? ∷ F*) (subst Value eq (Vσ x)))
... | inj₂ (a₀ , b₀ , refl , aeq , beq)
      with _ , _ , (_ , _ , ⊢a₀) , _ , _ ← IF.inv-app ⊢e
      with F₀ , arg₀ , refl , Feq , argeq ← frameApp-reflect Γ-S a₀ ⊢a₀ σ Vσ c F* (sym aeq)
      = (app₁ b₀ d (λ p → value-⋯⁻¹ σ Vσ b₀ (subst Value beq (V? p)))) ∷ F₀ , arg₀ , refl ,
        cong₂ L._∷_ (app₁-cong beq) Feq , argeq
frameApp-reflect Γ-S e₀ ⊢e σ Vσ c (app₂ e₃ d V? ∷ F*) eq
  with headApp σ e₀ eq
... | inj₁ (x , refl) = ⊥-elim (plugApp-not-value (app₂ e₃ d V? ∷ F*) (subst Value eq (Vσ x)))
... | inj₂ (a₀ , b₀ , refl , aeq , beq)
      ...
      = (app₂ a₀ d (λ p → value-⋯⁻¹ σ Vσ a₀ (subst Value aeq (V? p)))) ∷ F₀ , arg₀ , refl ,
        cong₂ L._∷_ (app₂-cong aeq) Feq , argeq
...
inv-ν-chanCx : ∀ {m} {Γ : Ctx m} {γ : Struct m} {B₁ B₂ P₀}
  ...
  → Σ[ Γ′ ∈ Ctx (sum B₁ + sum B₂ + m) ] Σ[ γ′ ∈ Struct (sum B₁ + sum B₂ + m) ]
      ChanCx Γ′ × (Γ′ ; γ′ ⊢ₚ P₀)
inv-ν-chanCx Γ-S ⊢νP
  with _ , _ , _ , _ , _ , _ , _ , C , C′ , p ← inv-ν ⊢νP =
