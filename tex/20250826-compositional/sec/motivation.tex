...
passing. Each use of the borrow notation
...
\lstinline{renderProfile} and \lstinline{renderPermissions} both render
one HTML node on a channel of type \lstinline{HtmlChannel; Ret}.
\begin{lstlisting}[numbers=left]
renderUser : User -> HtmlChannel; Ret -> Unit
renderUser user c =
  select Elem &c;
  send "section" &c;
  select Child &c;
...
renderUserRest : User -> ChildrenChannel; Ret -> Unit
renderUserRest user c =
  select Child &c;
  renderPermissions user &c;
  select Done c
\end{lstlisting}
...
specification that we can easily prove correct, before moving on to
...
operators. There are two splitting operators, one for local borrows and
...
\thecalculus. This translation is part of our implementation, as
well.

As an example, we use the local part of
\lstinline{renderUserRest} extended with closing the channel shown in
...
K(c) =
  ...
  renderPermissions user c3;
  let c5, c6 = lsplit c4 in
  select Done c5;
  close c6
\end{lstlisting}
...
\end{figure}
In the omitted prefix, the split on \lstinline{c} \emph{replaces} the
binder for \lstinline{c} with two fresh channel names \lstinline{c1}
and \lstinline{c2}. That is, each binding compartment of the
\lstinline{nu} contains a \emph{list} of binders and the operations
work on these channel names from left to right. In consequence, the
semantics does not allow any action on \lstinline{c2} before
\lstinline{c1} has been consumed.

On the client side, in the process $P_d$ that hangs on to \lstinline{d},
