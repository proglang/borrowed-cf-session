...
The session-type laws in \cref{fig:syntax-types} specify the equational theory
used by type conversion. Sequential composition has $\SSkip$ as unit and is
associative. Recursive types unfold equi-recursively. Continuations distribute
through internal and external choice, so a suffix can be exposed branchwise
before splitting or comparison. These laws are specification, not an
algorithm. In particular, the recursion law is the correct equi-recursive
specification, but it is not necessarily an executable definition of
conversion.

The duality laws specify the protocol offered by the opposite endpoint of a
fresh channel. Duality preserves $\SSkip$ and sequential composition, swaps
send with receive, swaps internal with external choice pointwise, and swaps
close with wait. For recursion, duality is specified by first unfolding the
recursive type and then dualizing the unfolded
body. \citet{DBLP:journals/corr/abs-2004-01322} discuss
implementations.
...
$\SDrop$, while recursive bounded sessions require all finite unfoldings to
...
$\SAcq$ and $\SDrop$ are not source protocols for $\CNew$; they are introduced
by remote splitting.
