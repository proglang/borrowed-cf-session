module BorrowedCF.Simulation.Support.Theorems.Drop where
...
open import BorrowedCF.Simulation.Support.Frames using (frame-plug*; frame*-⋯; frame-plug₁; ++ₛ-VSub; weaken-VSub)
open import BorrowedCF.Simulation.Support.TranslationProperties using (VChan; Value-subst; chanTriple-V)
open T using (inv-∥; inv-ν; inv-⟪⟫; BindCtx; BindCtx′; last; cons-ret/acq; cons-acq; nil; cons)
open import BorrowedCF.Simulation.Support.InvFrame using (inv-app; arg-type; strengthen-frame)
...
open import BorrowedCF.Simulation.Support.Theorems.B1VacProbe as VP using ()
...
head-noRet-last : ∀ {sF b}{Γ : Ctx (sum (suc b ∷ []))} →
  ...
  ∃[ s'' ] (Γ ﹫ 0F ≡ ⟨ s'' ⟩) × NoRet s''
head-noRet-last ns (last (cons s1 _ ¬sk s≃ _)) =
  s1 , refl , VP.noRet-;-fst (noRet-≃ (EqC.symmetric _≃𝕊_ s≃) ns)
