\newcommand{\theoldcalculus}{\textsc{BGV}\xspace}
\newcommand{\thecalculus}{\textsc{CSTB}\xspace}
\newcommand{\thertcalculus}{\textsc{RTSB}\xspace}

\newcommand{\metacolor}{\mathcolor{black}}
\newcommand{\ctxcolor}{\mathcolor{BurntOrange}} % RoyalBlue, ACMPurple
\newcommand{\typecolor}{\mathcolor{BurntOrange}} % RoyalBlue, ACMPurple
\newcommand{\termcolor}{\mathcolor{ForestGreen}} % Mahogany
\newcommand{\targettermcolor}{\mathcolor{Maroon}} % Sepia
...
\newcommand{\dircolor}{\mathcolor{ACMOrange}}
\newcommand{\mobcolor}{\mathcolor{ACMGreen}}
\newcommand{\csetcolor}{\mathcolor{ACMBlue}}
...
% Relations 
\newcommand{\reqDir}[2]{\text{DirCompat} \, #1 \, #2}
\newcommand{\reqEff}[3]{\text{EffCompat} \, #1 \, #2 \, #3}

% Grammars
\NewDocumentCommand\grmeq{s}{\; \Coloneqq \; \IfBooleanF{#1}{\;}}
\newcommand{\grmor}{\;\mid\;}
...
\NewMetaVar[\typecolor] \TU U
\NewMetaVar[\typecolor] \TVar \alpha
\NewMetaVar[\typecolor] \TVarb \beta
\NewMetaVar[\typecolor] \TVAR A
\newcommand*\TPair[1][\Dir]{\typecolor{\otimes}_{#1}}
\newcommand*\TPairOrd[2]{{#1} \TPair[\DirLeft] {#2}}
\newcommand*\TPairUnord[2]{{#1} \TPair[\DirUnord] {#2}}
\newcommand*\TSum[2]{\typecolor{{#1}+{#2}}}
\newcommand*\TVariant[2][\ell\in L]{\typecolor{\left\langle {#2} \right\rangle_{#1}}}
...
% Session Types
\let\S=\undefined % \S is already predefined and \NewMetaVar does not support \renewcommand.
...
\newcommand*\SDrop{\TypeKeyword{Drop}}
\newcommand*\SAcq{\TypeKeyword{Acq}}
\newcommand*\SEq{\metacolor{\simeq}}

\NewMetaVar[\typecolor] \AS  {{\S}^*}
\NewMetaVar[\typecolor] \AT  {{{\T\mkern2mu}^{*\mkern-2mu}}}
\NewMetaVar[\typecolor] \ATU {{{\TU}^{*\mkern-2mu}}}
...
\NewMetaVar[\ctxcolor] \SessCtx \Sigma
...
\newcommand*\CtxSeq{\ctxcolor{,}}
\newcommand*\CtxPar{\ctxcolor{\parallel}}
...
\NewDocumentCommand\CtxRestrict{s O{} m}{
  \Ctx[#2]\metacolor{{\mkern1.2mu}|_{\IfBooleanTF{#1}{#3}{\Fv(#3)}}}
}
\NewDocumentCommand\CtxJoin{s O{\Dir}}{\stackrel{\IfBooleanT{#1}{\smash}{#2}}{\metacolor{\bowtie}}}
\newcommand*\CtxAlgJoinCheck[3][]{
  \CtxRestrict[#1]{#2} \CtxJoin \CtxRestrict[#1]{#3} \SubCtx \Ctx[#1]
}
\newcommand*\CtxAlgSeqCheck[3][]{
  \CtxRestrict[#1]{#2} \CtxSeq \CtxRestrict[#1]{#3} \SubCtx \Ctx[#1]
}
\newcommand*\CaseJoinDir{\operatorname{joinDir}}

% Constraint sets
\NewMetaVar \CSet \Delta
%\NewMetaVar[\csetcolor] \CSet \Delta
%\newcommand*\CSEmpty{\csetcolor{\cdot}}
%\newcommand*\CSJoin{\csetcolor{,}}
\newcommand*\CSEmpty{\emptyset}
\newcommand*\CSJoin{\cup}
\newcommand*\CEq{\csetcolor{\simeq}}
\newcommand*\CSub{\csetcolor{\lesssim}}
\newcommand*\CMobile{\csetcolor{\Mobile}}
\newcommand*\CBounded{\csetcolor{\Bounded}}

\newcommand*\csred{\Longrightarrow}
...
\newcommand*\BSep{\procbindcolor{\|}}
\newcommand*\BWithSep[2]{\procbindcolor{#1\BSep#2}}
\newcommand*\BLeadSep[1]{\procbindcolor{\BSep#1}}
\newcommand*\BEmp{\procbindcolor{\varepsilon}}
...
\newcommand*\EAppD[3][{}]{\termcolor{({#2}\,@_{#1}\,{#3})}} % with direction
...
\newcommand*\ECase[3][]{\termcolor{\Keyword{case} {#2} \left\{ #3 \right\}_{#1}}}
\NewDocumentCommand\ECaseBranch{s}{\ECaseBranchImpl{#1}}
\NewDocumentCommand\ECaseBranchDef{s O{\ell}}{\ECaseBranchImpl{#1}{#2}{\EVar[#2]}{\E[#2]}}
\newcommand*\ECaseBranchImpl[4]{\IfBooleanT{#1}{\overline}{\termcolor{\EInj {#2} {#3} \Rightarrow {#4}}}}
...
\newcommand*\CDiscard{\TermKeyword{discard}}
...
\NewDocumentCommand \ChanPat {O{} m} {
  \termcolor{\mathcal{C}_{#1}[#2]}
}
...
% Evaluation Contexts
\NewMetaVar[\termcolor] \EC {\mathcal{E}}
...
\NewMetaVar[\targetproccolor] \TFC {\mathcal{G}}
...
\NewMetaVar[\targetproccolor] \TProcQ Q
\NewMetaVar[\targetproccolor] \TProcR R
...
\newcommand*\TPFlag[2]{\targetproccolor{#1 \mapsto #2}}
\newcommand*\TPSync[2][\TEVarz]{\targetproccolor{(\TBindSync\TPFlag{#1}{#2})}}

\newcommand* \TBindNu   {\nu}
\newcommand* \TBindSync {\varphi}

\NewMetaVar[\targetproccolor] \Flag \phi
\newcommand*\FlagInit{\targetproccolor{\textsf{drop}}}
\newcommand*\FlagDropped{\targetproccolor{\textsf{acq}}}
\newcommand*\FlagAcquired{\targetproccolor{\textsf{done}}}

\newcommand*\BindFlag[1]{\metacolor{\operatorname{flag}\ifstrempty{#1}{}{(#1)}}}
...
% New Types
\NewDocumentCommand \SNew {s} {\operatorname{\mathsf{new}\IfBooleanT{#1}{'}}}

% Terminated Session Types
\newcommand*\Bounded{\operatorname{\mathsf{bnd}}}
\newcommand*\Mobile{\operatorname{\mathsf{mbl}}}
...
\NewDocumentCommand \InferType {O{\Ctx} m m m O{\CSet}}
  {
    \ctxcolor{#1} \vdash #2 \Rightarrow #3 \mid #4 \uparrow {#5}
  }
\NewDocumentCommand \CheckType {O{\Ctx} m m m O{\CSet}}
  {
    \ctxcolor{#1} \vdash #2 \Leftarrow #3 \mid #4 \uparrow {#5}
  }

% Terminated types
\NewDocumentCommand \TerminatedType {m}
  {
    \vdash #1 \checkmark
  }

\NewDocumentCommand \NonTerminatedType {m}
  {
    \nvdash #1 \checkmark
  }

% Type Normalisation
\NewDocumentCommand \ConcatType {m m}
  {
    #1 \mathbin{\text{\ttfamily\upshape ++}} #2
  }

\NewDocumentCommand \NormaliseType {m m}
  {
    #1 \Downarrow #2
  }
...
\newcommand{\EReduce}{\mathrel{\Reduces_e}}
\newcommand{\PReduce}{\mathrel{\Reduces_p}}
\newcommand{\TPReduce}{\mathrel{\Reduces'_{\!p}}}
...
% Weak bisimilarity used for the operational correspondence: it quotients the
% administrative drop/acquire steps and the structural symmetries under which
% the process translation is not a homomorphism.
\newcommand*\Bisim{\mathrel{\metacolor{\approx}}}
...
% Process translation as a function: the run-time configuration of a well-typed
% source process #2 under the value substitution #1 (default \Sub).
\newcommand*\TransP[2][\Sub]{\metacolor{\llbracket #2 \rrbracket_{#1}}}
\NewDocumentCommand\TransBind{O{\E} m o}{\stackrel{\targettermcolor{#1}}{\TrRel{}} #2 \IfValueT{#3}{\mid #3}}
...
% Translation of session types to FreeST types (Appendix).
\newcommand*\tofreest[1]{\llbracket #1 \rrbracket}
...
\newcommand*\AgdaBird{%
  \raisebox{-0.28ex}{\includegraphics[height=1.6ex]{assets/agda-bird.png}}%
}
\newcommand*\AgdaProofIcon{\ensuremath{\blacktriangleright}}
\newcommand*\AgdaProofRef[2]{%
  \hfill{\footnotesize\textcolor{gray}{\texttt{#1}:\texttt{#2}}}%
}
