...
--
-- `Pf4` (F4 a) and `Pf4b` (F4 b) below are GENUINE COUNTEREXAMPLES: well-typed
-- closed processes -- `f4-typing` and `f4b-typing` are checked here -- on which
-- the soup steps while no typed rule applies, and whose soup reduct is the
-- flattening of no well-typed process at all (PLAN.md §4, §5).
--
-- `Pf4c` (F4 c) is the near miss: the soup step exists, but `⊢ᴮ` rejects its
-- binder shape outright, so acquire is no counterexample -- there the typed
-- rules are complete.
...
------------------------------------------------------------------------
-- Typability of the F4 (a) probe: `Pf4` IS well typed.
...
-- The tail of the left endpoint: the group the partner may acquire once the
-- boundary in front of it has been released.
f4-tail-block : 𝐓.BindCtx (acq ; skip) (1 ∷ []) (⟨ acq ⟩ ∷ [])
f4-tail-block = 𝐓.last (𝐓.cons acq skip (λ { (() ; _) }) ≃-refl (𝐓.nil skip))

-- The left endpoint: an initial `acq` boundary, then the two halves of the
-- lsplit, then a second boundary and the remainder.  That second boundary sits
-- directly in front of the next group's `acq`, so the group's own remainder
-- `s₂` is `skip` -- which is exactly what the `s≃` premise records.
f4-C1 :
  𝐓.BindCtx (skip ; end ‼) (0 ∷ 2 ∷ 1 ∷ [])
    ((⟨ acq ; end ‼ ⟩ ∷ ⟨ ret ⟩ ∷ []) V.++ (⟨ acq ⟩ ∷ []))
f4-C1 =
  𝐓.cons-acq
    (𝐓.cons-ret/acq (acq ; end ‼)
      (≃-trans ≃-skipʳ (≃-; ≃-refl (≃-sym ≃-skipˡ)))
...
-- `Pf4` is well typed -- so the `RUS-Drop` step above, which no typed rule can
-- match, is a genuine counterexample to the naive backward proposition:
f4-typing : [] ; [] ⊢ₚ Pf4
f4-typing =
  𝐓.TP-Res skip ‼ f4-B1 f4-B2 f4-C1 f4-C2
    (𝐓.TP-Weaken f4-≼
      (𝐓.TP-Par
...
-- The second group: the `⟨ skip ⟩` handle that an `lsplit` peeled off
-- `⟨ acq ; end ‼ ⟩ ≃ ⟨ skip ; (acq ; end ‼) ⟩`, followed by the live half.
f4b-second-group :
...
-- The left endpoint: the `ret` handle of the first group, the group boundary
-- in front of the second group's `acq`, and then that second group.
f4b-C1 :
  𝐓.BindCtx (skip ; end ‼) (1 ∷ 2 ∷ [])
    ((⟨ ret ⟩ ∷ []) V.++ (⟨ skip ⟩ ∷ ⟨ acq ; end ‼ ⟩ ∷ []))
f4b-C1 =
...
-- `Pf4b` is well typed too, so the `RUS-Discard` step above is a second soup
-- step that the typed calculus cannot match:
f4b-typing : [] ; [] ⊢ₚ Pf4b
f4b-typing =
  𝐓.TP-Res skip ‼ (_ ∷ []) [] f4b-C1 f4-C2
    (𝐓.TP-Weaken f4b-≼
