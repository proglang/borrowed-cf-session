...
acq-goB :
  ...
  → Σ[ P′ ∈ T.Proc m ] Σ[ Q′ ∈ U.Proc n ]
      ...
      × ( (U.ν ((U.⟪ F [ ((` 0F) ⊗ (` 1F)) ⊗ e' ]* ⟫ U.∥ P₁) U.⋯ₚ ⦅ * ⦆ₛ) ≡ Q′)
          Sum.⊎ (U.ν ((U.⟪ F [ ((` 0F) ⊗ (` 1F)) ⊗ e' ]* ⟫ U.∥ P₁) U.⋯ₚ ⦅ * ⦆ₛ) UR.─→ₚ Q′) )
      × (Q′ ≈ U[ P′ ] σ)
...
acq-goB (zero ∷ c ∷ []) (b₂ ∷ []) σ Vσ Γ-S P₀ F {e'} {P₁} ⊢P bodyeq
  ...
  with x0 , e* ← acq-image-0F c' b₂ σ Vσ x argeq
  with refl ← x0
  with refl ← e*
  = P′ , _ , TR.R-Acq {b₁ = c'} {B₁ = []} {B₂ = b₂ ∷ []} {P = Pr} {E = F₀} ◅ ε
  , Sum.inj₁ refl , ≋⇒≈ (≡→≋ recon)
  where
    νσ' = νσ (suc c') b₂ σ
    ...
    recon : U.ν ((U.⟪ F [ ((` 0F) ⊗ (` 1F)) ⊗ * ]* ⟫ U.∥ P₁) U.⋯ₚ ⦅ * ⦆ₛ) ≡ U[ P′ ] σ
