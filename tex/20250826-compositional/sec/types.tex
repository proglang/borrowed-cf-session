...
\Cref{fig:constant-typing} assigns closed types to constants. Channel
...
constants $\CDrop$ and $\CAcq$ eliminate the 
corresponding marker types. Forking expects a mobile impure computation from unit to
...
type $\T$. The effect
annotation $\Eff$ is an upper bound: pure expressions may be used where impure
...
The ordered application rules enforce the expected purity side condition:
...
function must be pure. Any other application can be evaluated
