...
follows only from an assumption or from stronger structural constraints;
failure to prove $\alpha\SEq\SSkip$ is not evidence for $\NonSkip\alpha$.  At
a type application, a qualification
...
\paragraph{No equality qualification.}
Type equivalence remains an ordinary judgment of the calculus, not a
qualification that can be assumed for a quantified type variable.  A
polymorphic interface should expose a required type decomposition directly.
For example, the argument type of a wrapper around local splitting can inline
the sequential structure (arrow annotations are shown explicitly):
\begin{align*}
  \mathsf{splitAs}:{}&
    \typecolor{\forall(\beta,\gamma:\KSess).}\\[-.3ex]
  &\quad \typecolor{\NonSkip\gamma\Rightarrow}\\[-.3ex]
  &\quad
      \TArr{\MobMobile}{\DirNone,\EffPure}
        {\SSeq\beta\gamma}{\TPairOrd\beta\gamma},\\
  \mathsf{splitAs}{[\beta]}{[\gamma]}\,c ={}&
    \EApp{\CSplit{[\beta]}{[\gamma]}}c.
\end{align*}
The $\NonSkip\gamma$ qualification discharges the side condition of local
splitting, while the argument already has the required sequence type.  More
generally, a use of an equivalent type is handled by placing the existing
\RuleName{T-Conv} rule at the relevant typing derivation.  There is no need to
create an equality witness in the qualification solver.
...
predicate decisions; deciding $\NonSkip$ uses CFST equivalence with $\SSkip$.
When a type contains type variables, entailment uses only assumptions and the
...
Thus $\Unr$ and $\Mobile$ apply to value types, whereas $\Bounded$, $\SNew$,
$\Dualable$, and $\NonSkip$ apply to session types.  By \RuleName{K-Var}, the
kinding premises are
...
For scoping, write $\TyCtx\vdash\T\SEq\TU$ for the paper's ordinary type
equivalence judgment restricted to types well formed in $\TyCtx$.
Qualification assumptions in $\TyCtx$ do not add equations to this judgment.
...
the inherited rules is otherwise unchanged.  Predicate side conditions become
entailments, whereas type equivalence remains a separate judgment.
...
\begin{array}{lll}
  ...
  \RuleName{T-Conv}:& \TyCtx\vdash\T\SEq\TU,
    &\text{conversion uses ordinary type equivalence}.
\end{array}
\]
...
\item \emph{Entailment substitution.}  If
  ...
  $\TyCtx\Entails\Prop{[\TU/\alpha]}$, where $\alpha$ is fresh for
  $\TyCtx$.

  \emph{Proof.}
  By induction on the derivation of
  $\TyCtx,\alpha:\Kind,\Qual\Entails\Prop$.
  In the \RuleName{Q-Assume} case, an assumption inherited from $\TyCtx$
  does not mention $\alpha$, by well-formedness and freshness, and remains
  available by \RuleName{Q-Assume}.  An assumption contributed by $\Qual$
  is a conjunct of $\Qual$; after substitution it follows from
  $\TyCtx\Entails\Qual{[\TU/\alpha]}$, since entailment of conjunctions is
  defined componentwise.

  Every structural Horn clause commutes with substitution.  Apply the
  induction hypotheses to its entailment premises and reapply the same
  clause; the standard kinding-substitution lemma preserves any implicit
  kinding side conditions.  This covers the inherited monomorphic predicate
  rules and the structural dualability postulates.  The only exceptional
  postulates are the variable-specific ones.  If their subject is a variable
  other than $\alpha$, its declaration remains in $\TyCtx$, so the same
  postulate applies after using the induction hypothesis on its premise.  For
  \RuleName{Q-Unr-Var-Mobile} with subject $\alpha$, the induction hypothesis
  yields $\TyCtx\Entails\Unr\TU$, and admissible
  \RuleName{Q-Unr-Mobile} yields $\TyCtx\Entails\Mobile\TU$.  For
  \RuleName{Q-New-Var-Dual}, kinding forces $\Kind=\KSess$; the induction
  hypothesis yields $\TyCtx\Entails\SNew\TU$, and admissible
  \RuleName{Q-New-Dual} yields $\TyCtx\Entails\Dualable\TU$.

  The derived rules require no additional cases: expanding their
  admissibility derivations reduces them to the cases above.  In particular,
  the subject of \RuleName{Q-NonSkip-Ground} is closed, so substitution leaves
  it unchanged.  This completes the induction.
...
equivalence procedure where required by predicates such as $\NonSkip$.
Ordinary conversion remains outside qualification entailment and can be
inserted explicitly in a checking derivation.  The algorithmic fixed-point
rule additionally checks the supplied type of the recursive variable; it does
not infer a polymorphic recursive type.
...
$\SDual\alpha$ well formed.  The remaining predicates are directly observed
by the declarative rules and should not be collapsed into mobility alone.
