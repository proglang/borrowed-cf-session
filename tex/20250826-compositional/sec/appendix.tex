...
\begin{align*}
  ...
  \FC \grmeq
  & \PExp \EC
    \tag{Process-local evaluation contexts}
  \\
  \CC \grmeq
  & \ECEmpty
    \grmor \PPar{\CC}{\Proc}
...
\begin{definition}[Poised process]
  \label{def:poised}
  A process $\PExp\E$ is \emph{poised} when $\E = \ECapp{\EApp\C\V}$ for a constant
...
\begin{lemma}[Process expression progress]
  \label{lem:process-expression-progress}
...
\begin{proof}
  ...
  poised process. A value of type $\BUnit$ in a session context is $\CUnit$ by
  canonical forms.
\end{proof}
...
\begin{lemma}[Synchronization]
  ...
  $\EVar : \SSeq\S{\S*}$ and $\EVary : \SDual{(\SSeq\S{\S*})}$. If one process in
...
\begin{proof}
  ...
  pruning of $\PExp\CUnit$ processes by \textsc{Par-Unit}.

  \proofcase{\textsc{TP-Expr}, $\Proc = \PExp\E$, $\HasType[\EffImpure]\SessCtx\E\BUnit$.}
  By \cref{lem:process-expression-progress}:
  \begin{itemize}
  ...
  \begin{itemize}
    ...
    \item $\ProcQ$ blocked. Up to pruning, $\ProcQ$ is a parallel composition of
      poised processes and blocked restrictions, the latter closed by the
      induction hypothesis. Consider the poised processes acting on the endpoints
      bound here.
      \begin{enumerate}
        \item Some process is poised with $\CDrop,\CDiscard,\CSplit,\CSSplit$ on a
