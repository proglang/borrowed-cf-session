...
module BorrowedCF.Simulation.Support.AcqInv where
...
open import BorrowedCF.Simulation.Support.HandleLinear using (¬Skips⇒¬Unr-seq)
...
acq-app-nonUnr : ∀ {N} {Γ : Ctx N} {β : Struct N} {dir} {x : 𝔽 N} {T ϵ}
  → Γ ; β ⊢ K `acq ·⟨ dir ⟩ (` x) ∶ T ∣ ϵ → ¬ Unr (lookup Γ x)
