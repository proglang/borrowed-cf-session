...
value⇒pure V (T-Conv T≤ ϵ≤ x) = T-Conv T≤ ≤ϵ-refl (value⇒pure V x)
...
module _ (Γ-S : ChanCx Γ) where
  ...
  inv-`⊤ V (T-Conv T≤ ϵ≤ e) rewrite ≲⁻¹-`⊤ʳ T≤ = inv-`⊤ V e
  ...
  inv-arr V (T-Conv T≤ ϵ≤ e)
    with _ , _ , _ , refl , arr ϵ′≤ , dom≤ , cod≤ ← ≲⁻¹-→ʳ T≤
    with _ , _ , _ , dom′≤ , cod′≤ , ϵ″≤ , x ← inv-arr V e
    = _ , _ , _ , ≲-trans dom≤ dom′≤ , ≲-trans cod′≤ cod≤ , ≤ϵ-trans ϵ″≤ ϵ′≤ , x
  inv-arr {a = a} V (T-Weaken γ≤ e)
    with inv-arr V e
  ... | _ , _ , _ , dom′≤ , cod′≤ , ϵ″≤ , inj₁ x
    = _ , _ , _ , dom′≤ , cod′≤ , ϵ″≤ , inj₁ x
  ... | _ , _ , _ , dom′≤ , cod′≤ , ϵ″≤ , inj₂ (_ , eq , x)
    = _ , _ , _ , dom′≤ , cod′≤ , ϵ″≤ , inj₂ (_ , eq , T-Weaken (≼-join (Arr.dir a) (≼-refl refl) (𝐂.≼-𝐂wk γ≤)) x)
  ...
  inv-⊗ V (T-Conv T≤ ℙ≤ϵ x)
    with _ , _ , refl , ≤₁ , ≤₂ ← ≲⁻¹-⊗ʳ T≤
    = let _ , _ , _ , _ , eq , γ≤′ , x₁ , x₂ = inv-⊗ V x in
      _ , _ , _ , _ , eq , γ≤′ , T-Conv ≤₁ ℙ≤ϵ x₁ , T-Conv ≤₂ ℙ≤ϵ x₂
  inv-⊗ V (T-Weaken γ≤ x)
    = let _ , _ , _ , _ , eq , γ≤′ , x₁,x₂ = inv-⊗ V x in
  ...
  inv-session : Value e → Γ ; γ ⊢ e ∶ ⟨ s ⟩ ∣ ϵ →
    ∃[ s′ ] ∃[ x ] e ≡ ` x × Γ x ≡ ⟨ s′ ⟩ × Γ ∶ ` x ≼ γ × s′ ≲ s
  inv-session V (T-Var x T-eq) = _ , x , refl , T-eq , ≼-refl refl , ≲-refl
  inv-session V (T-Conv T≤ ϵ≤ x)
    with _ , refl , s≤ ← ≲⁻¹-⟨⟩ʳ T≤
    = let _ ,  _ , eq-e , eq-x , γ≤ , s′≤ = inv-session V x in
      _ , _ , eq-e , eq-x , γ≤ , ≲-trans s′≤ s≤
  inv-session V (T-Weaken γ≤ x)
    = let _ ,  _ , eq-e , eq-x , γ≤′ , s≤ = inv-session V x in
      _ , _ , eq-e , eq-x , ≼-trans γ≤′ γ≤ , s≤
  ...
  Unr×Value⇒UnrCx U V (T-Conv T≤ ϵ≤ e) = Unr×Value⇒UnrCx (unr-respects-≳ T≤ U) V e
  ...
  Mobile×Value⇒MobCx m V (T-Conv T≤ ϵ≤ e) = {!Mobile×Value⇒MobCx m V e!}
