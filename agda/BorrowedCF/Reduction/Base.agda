...
data _;_⊢_∶_∣_⟶_∣_ (Γ : Ctx n) : CxPat n → Frame n → 𝕋 → Eff → 𝕋 → Eff → Set where
  ...
  TF-·□ : {V₁ : Value e₁} →
    ...
    (appRight : Arr.IsR a → ϵ₁ ≡ ϵ × ϵ₂ ≡ ℙ) →
    Γ ; γ ⊢ e₁ ∶ T ⟨ a ⟩→ U ∣ ϵ₁ →
    Γ ; 𝒫[ flipDir (Arr.dir a) , γ ] ⊢ V₁ ·□ ∶ T ∣ ϵ₂ ⟶ U ∣ ϵ
...
TF-□· ≤ₐ appPar appLeft appRight x ⊢⋯ᶠ ⊢ϕ = TF-□· ≤ₐ appPar appLeft appRight (x ⊢⋯ ⊢ϕ)
TF-·□ ≤ₐ appPar appLeft appRight x ⊢⋯ᶠ ⊢ϕ = TF-·□ ≤ₐ appPar appLeft appRight (x ⊢⋯ ⊢ϕ)
...
⊢[]⁻¹ (□· e₂) e x with inv-· x
... | a , _ , _ , _ , ≤γ , ≤ₐ , T-AppUnr a-unr x₁ x₂ =
  _ , _ , _ , _ , _ , _ , ≤γ , ≃-refl , ≤ϵ-refl
    , TF-□· ≤ₐ (λ z → refl , refl)
               ...
               x₂
    , x₁
... | a , _ , _ , _ , ≤γ , ≤ₐ , T-AppLin a-par x₁ x₂ =
  _ , _ , _ , _ , _ , _ , ≤γ , ≃-refl , ≤ϵ-refl
    , TF-□· ≤ₐ (λ z → refl , refl)
               ...
               x₂
    , x₁
... | a , _ , _ , _ , ≤γ , ≤ₐ , T-AppLeft aL x₁ x₂ =
  _ , _ , _ , _ , _ , _ , ≤γ , ≃-refl , ≤ϵ-refl
    , TF-□· ≤ₐ (λ z → case sym z ■ aL of λ())
               (λ z → refl , refl)
               (λ z → case sym z ■ aL of λ())
               x₂
    , x₁
... | a , _ , _ , _ , ≤γ , ≤ₐ , T-AppRight aR x₁ x₂ =
  _ , _ , _ , _ , _ , _ , ≤γ , ≃-refl , ≤ϵ-refl
    , TF-□· ≤ₐ (λ z → case sym z ■ aR of λ())
