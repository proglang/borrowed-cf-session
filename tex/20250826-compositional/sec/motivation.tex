...
TODO: basics of CFST (monoidal structure of skip and composition;
distributivity of composition over choice)
...
continuation. The return type is \lstinline{Skip}, which indicates a
channel without interaction. Values of type \lstinline{Skip} are
...
\lstinline{renderUserProfile} and \lstinline{renderMsgArea} both render
...
renderUser user c =
  ...
  fork (lambda() -> renderUserProfile user &c);
...
renderUserProfile : User -> HtmlChannel;Ret -> Unit
renderUserProfile user c = ... drop c
\end{lstlisting}
...
In this case, the borrow under the \lstinline{fork} is a \emph{remote
  borrow}. Such a borrow must be handled differently: the
...
implemented with the \lstinline{acquire} operation. It blocks until the
borrow created by the fork is completed, i.e., until
\lstinline{renderUserProfile} finishes sending on the borrowed prefix
of the channel. The function indicates this situation by calling
\lstinline{drop} on the channel. Calling \lstinline{drop} sets a flag which unblocks a
waiting \lstinline{acquire} on the suffix of the channel.
Once it is unblocked, the \lstinline{acquire}
operation returns the current channel end for further operations. The
communication between \lstinline{drop} and \lstinline{acquire} is
\emph{asynchronous} whereas normal communication is synchronous in \thecalculus.
...
\emph{binding group}, which binds one or more names.

 To keep the trace short, the
...
We implement the high-level calculus shown so far by translation into
a low-level run-time calculus, \thertcalculus. The run-time calculus
is a concurrent 
...
$in \otimes c \otimes out$, where $c$ is a primitive channel name and $in$
and $out$ point to  optional synchronization processes, for incoming and
outgoing synchronizations. A synchronization
process $(\varphi z \mapsto \phi)P$ carries one of three different states
$\phi$: acq, drop, and done. This process binds name $z$ which is scoped
over $P$. 

The incoming synchronization process indicates whether the channel has to
be acquired before using it. The outgoing synchronization process
tells us whether the channel must be dropped after use.

Allocating a channel creates a primitive channel and one
synchronization channel for each channel end. Channel ends start out
in state BB, which means they are enabled for an \lstinline{acquire}
operation. Hence, before running a protocol on a channel, we first
have to \lstinline{acquire} it. Acquiring a channel sets its incoming
synchronization flag to a ready state CC, which is expected by all
normal operations. 
