...
module BorrowedCF.Simulation.InvFrame where
...
open import BorrowedCF.Simulation.HandleLinear using (¬Skips⇒¬Unr-seq)
open import BorrowedCF.Context.Join using (biasedDir; join)
import BorrowedCF.Context.Substitution as 𝐂S
open import BorrowedCF.Simulation.Strengthen using (Inverter; inv↑; strengthen-Tm-gen)
...
inv-app (T-AppUnr {γ₁ = γ₁} {γ₂ = γ₂} _ ⊢e₁ ⊢e₂) =
  γ₁ , γ₂ , (_ , _ , ⊢e₁) , (_ , _ , ⊢e₂) , λ h _ → ≤-refl
inv-app (T-AppLin {γ₁ = γ₁} {γ₂ = γ₂} _ ⊢e₁ ⊢e₂) =
  γ₁ , γ₂ , (_ , _ , ⊢e₁) , (_ , _ , ⊢e₂) , λ h _ → ≤-refl
inv-app (T-AppLeft {γ₁ = γ₁} {γ₂ = γ₂} _ ⊢e₁ ⊢e₂) =
  γ₁ , γ₂ , (_ , _ , ⊢e₁) , (_ , _ , ⊢e₂) , λ h _ → ≤-reflexive (+-comm (count h γ₁) (count h γ₂))
inv-app (T-AppRight {γ₁ = γ₁} {γ₂ = γ₂} _ ⊢e₁ ⊢e₂) =
  γ₁ , γ₂ , (_ , _ , ⊢e₁) , (_ , _ , ⊢e₂) , λ h _ → ≤-refl
...
inv-pair : ∀ {N} {Γ : Ctx N} {α : Struct N} {e₁ e₂ : Tm N} {T ϵ}
  ...
  → Σ[ α₁ ∈ Struct N ] Σ[ α₂ ∈ Struct N ]
      ...
      × (∃[ T₂ ] ∃[ ϵ₂ ] Γ ; α₂ ⊢ e₂ ∶ T₂ ∣ ϵ₂)
      × ((h : 𝔽 N) → ¬ Unr (Γ h) → count h α₁ + count h α₂ ≤ count h α)
inv-pair (T-Pair p/s {γ₁ = γ₁} {γ₂ = γ₂} ⊢e₁ ⊢e₂ _) =
  γ₁ , γ₂ , (_ , _ , ⊢e₁) , (_ , _ , ⊢e₂) ,
...
inv-seq : ∀ {N} {Γ : Ctx N} {α : Struct N} {e₁ e₂ : Tm N} {T ϵ}
  ...
  → Σ[ α₁ ∈ Struct N ] Σ[ α₂ ∈ Struct N ]
      ...
      × (∃[ T₂ ] ∃[ ϵ₂ ] Γ ; α₂ ⊢ e₂ ∶ T₂ ∣ ϵ₂)
      × ((h : 𝔽 N) → ¬ Unr (Γ h) → count h α₁ + count h α₂ ≤ count h α)
inv-seq (T-LetUnit p/s {γ₁ = γ₁} {γ₂ = γ₂} ⊢e₁ ⊢e₂) =
  γ₁ , γ₂ , (_ , _ , ⊢e₁) , (_ , _ , ⊢e₂) ,
...
fn-lsplit-dom (T-Const (`lsplit {s′ = s′} ¬Ss)) = ¬Ss , s′ , ≃-refl
...
lsplit-app-nonUnr d = go d
  where
    go : ∀ {N} {Γ : Ctx N} {β : Struct N} {s : 𝕊 0} {x : 𝔽 N} {T ϵ}
       → Γ ; β ⊢ K (`lsplit s) · (` x) ∶ T ∣ ϵ → ¬ Unr (Γ x)
    go (T-AppUnr _ ⊢fn ⊢arg) u =
      let ¬Ss , s′ , eq = fn-lsplit-dom ⊢fn
      in ¬Skips⇒¬Unr-seq ¬Ss (unr-≃ (≃-trans (arg-type ⊢arg) (≃-sym eq)) u)
    go (T-AppLin _ ⊢fn ⊢arg) u =
      let ¬Ss , s′ , eq = fn-lsplit-dom ⊢fn
      in ¬Skips⇒¬Unr-seq ¬Ss (unr-≃ (≃-trans (arg-type ⊢arg) (≃-sym eq)) u)
    go (T-AppLeft _ ⊢fn ⊢arg) u =
      let ¬Ss , s′ , eq = fn-lsplit-dom ⊢fn
      in ¬Skips⇒¬Unr-seq ¬Ss (unr-≃ (≃-trans (arg-type ⊢arg) (≃-sym eq)) u)
    go (T-AppRight _ ⊢fn ⊢arg) u =
      let ¬Ss , s′ , eq = fn-lsplit-dom ⊢fn
      in ¬Skips⇒¬Unr-seq ¬Ss (unr-≃ (≃-trans (arg-type ⊢arg) (≃-sym eq)) u)
...
fn-rsplit-dom (T-Const (`rsplit {s′ = s′} ¬Ss)) = ¬Ss , s′ , ≃-refl
