module BorrowedCF.Simulation.ForwardSoup.LSplit where
...
private
  ...
  leftGroup′ : Typed.BindGroup → ℕ → ℕ → Typed.BindGroup → Typed.BindGroup
  leftGroup′ B₁ q b₁ B₂ = B₁ ++ (q + suc (suc b₁)) ∷ B₂
  ...
  lsplitTarget :
    ...
    Typed.Proc (sum (leftGroup B₁ q b₁ B₂) + sum B + 0) →
    Typed.Proc 0
  lsplitTarget B₁ B₂ B q b₁ s E P =
    let module 𝐒 = Source.SplitRenamings B₁ B₂ (sum B) in
    Typed.ν (leftGroup′ B₁ q b₁ B₂) B
      (Typed.⟪ plug*ˢ (E SourceReduction.⋯ᶠ* 𝐒.lwk)
          ((Source.` (𝐒.atk (q ↑ʳ 0F))) Source.⊗
           (Source.` (𝐒.atk (q ↑ʳ 1F)))) ⟫
       Typed.∥ (P Typed.⋯ₚ 𝐒.lwk))
  ...
  dlwkq : ∀ B₁ q b₁ B₂ →
    𝔽 (sum (B₁ ++ (q + suc b₁) ∷ B₂)) →
    𝔽 (sum (B₁ ++ (q + suc (suc b₁)) ∷ B₂))
  dlwkq [] zero b₁ B₂ i = (Source.weakenᵣ Source.↑* 1) i
  dlwkq [] (suc q) b₁ B₂ zero = zero
  dlwkq [] (suc q) b₁ B₂ (suc i) =
    suc (dlwkq [] q b₁ B₂ i)
  dlwkq (a ∷ B₁) q b₁ B₂ i =
    [ (λ p → p ↑ˡ sum (B₁ ++ (q + suc (suc b₁)) ∷ B₂))
    , (λ r → a ↑ʳ dlwkq B₁ q b₁ B₂ r)
    ]′ (Fin.splitAt a i)
  ...
  dlwkq-lo : ∀ B₁ q b₁ B₂
    (j : 𝔽 (sum (B₁ ++ (q + suc b₁) ∷ B₂))) →
    Fin.toℕ j Nat.< sum B₁ + q + 1 →
    Fin.toℕ (dlwkq B₁ q b₁ B₂ j) ≡ Fin.toℕ j
  dlwkq-lo [] zero b₁ B₂ j lt =
    toℕ-↑*-lt Source.weakenᵣ 1 j lt
  dlwkq-lo [] (suc q) b₁ B₂ zero lt = refl
  dlwkq-lo [] (suc q) b₁ B₂ (suc j) lt =
    cong suc (dlwkq-lo [] q b₁ B₂ j (Nat.s≤s⁻¹ lt))
  dlwkq-lo (a ∷ B₁) q b₁ B₂ j lt
    with Fin.splitAt a j in split
  ... | inj₁ p =
    Fin.toℕ-↑ˡ p _
    ■ sym jℕ
    where
    jℕ : Fin.toℕ j ≡ Fin.toℕ p
    jℕ =
      cong Fin.toℕ
        (sym (Fin.join-splitAt a
          (sum (B₁ ++ (q + suc b₁) ∷ B₂)) j)
        ■ cong (Fin.join a (sum (B₁ ++ (q + suc b₁) ∷ B₂))) split)
      ■ Fin.toℕ-↑ˡ p (sum (B₁ ++ (q + suc b₁) ∷ B₂))
  ... | inj₂ r =
    Fin.toℕ-↑ʳ a (dlwkq B₁ q b₁ B₂ r)
    ■ cong (a +_) (dlwkq-lo B₁ q b₁ B₂ r boundr)
    ■ sym jℕ
    where
    jℕ : Fin.toℕ j ≡ a + Fin.toℕ r
    jℕ =
      cong Fin.toℕ
        (sym (Fin.join-splitAt a
          (sum (B₁ ++ (q + suc b₁) ∷ B₂)) j)
    ...
    assoc3 =
      cong (Nat._+ 1) (Nat.+-assoc a (sum B₁) q)
      ■ Nat.+-assoc a (sum B₁ + q) 1
    boundr : Fin.toℕ r Nat.< sum B₁ + q + 1
    boundr =
      Nat.+-cancelˡ-< a (Fin.toℕ r) (sum B₁ + q + 1)
        (subst (Nat._< a + (sum B₁ + q + 1)) jℕ
          (subst (Fin.toℕ j Nat.<_) assoc3 lt))

  dlwkq-hi : ∀ B₁ q b₁ B₂
    (j : 𝔽 (sum (B₁ ++ (q + suc b₁) ∷ B₂))) →
    sum B₁ + q + 1 Nat.≤ Fin.toℕ j →
    Fin.toℕ (dlwkq B₁ q b₁ B₂ j) ≡ suc (Fin.toℕ j)
  dlwkq-hi [] zero b₁ B₂ j h =
    toℕ-↑*-ge Source.weakenᵣ 1 j h
    ■ cong (1 +_) (cong suc (toℕ-reduce≥ j h))
    ■ cong suc (Nat.m+[n∸m]≡n h)
  dlwkq-hi [] (suc q) b₁ B₂ zero h =
    ⊥-elim (Nat.<-irrefl refl
      (Nat.<-≤-trans (Nat.s≤s Nat.z≤n) h))
  dlwkq-hi [] (suc q) b₁ B₂ (suc j) h =
    cong suc (dlwkq-hi [] q b₁ B₂ j (Nat.s≤s⁻¹ h))
  dlwkq-hi (a ∷ B₁) q b₁ B₂ j h
    with Fin.splitAt a j in split
  ... | inj₁ p =
    ⊥-elim (Nat.<-irrefl refl
      (Nat.<-≤-trans
        (Nat.<-≤-trans
          (subst (Nat._< a) (sym jℕ) (Fin.toℕ<n p))
          (Nat.m≤m+n a (sum B₁ + q + 1)))
  ...
  ... | inj₂ r =
    Fin.toℕ-↑ʳ a (dlwkq B₁ q b₁ B₂ r)
    ■ cong (a +_) (dlwkq-hi B₁ q b₁ B₂ r boundr)
    ■ Nat.+-suc a (Fin.toℕ r)
    ■ cong suc (sym jℕ)
    where
    jℕ : Fin.toℕ j ≡ a + Fin.toℕ r
    ...
    boundr =
      Nat.+-cancelˡ-≤ a (sum B₁ + q + 1) (Fin.toℕ r)
  ...
  positive-flag : ∀ q b →
    Translation.ϕ[ q + suc b ] ≡
    Translation.ϕ[ q + suc (suc b) ]
  ...
  bindFlags-lsplit : ∀ B₁ B₂ q b →
    bindFlags (leftGroup B₁ q b B₂) ≡
    bindFlags (leftGroup′ B₁ q b B₂)
  bindFlags-lsplit [] [] q b = refl
  bindFlags-lsplit [] (b₂ ∷ B₂) q b =
    cong (λ z → z ∷ bindFlags (b₂ ∷ B₂))
      (positive-flag q b)
  bindFlags-lsplit (b₀ ∷ []) B₂ q b =
    cong (Translation.ϕ[ b₀ ] ∷_)
      (bindFlags-lsplit [] B₂ q b)
  bindFlags-lsplit (b₀ ∷ b₁ ∷ B₁) B₂ q b =
    cong (Translation.ϕ[ b₀ ] ∷_)
      (bindFlags-lsplit (b₁ ∷ B₁) B₂ q b)

  syncs-lsplit : ∀ B₁ B₂ q b →
    Translation.syncs (leftGroup B₁ q b B₂) ≡
    Translation.syncs (leftGroup′ B₁ q b B₂)
  syncs-lsplit [] [] q b = refl
  syncs-lsplit [] (b₂ ∷ B₂) q b = refl
  syncs-lsplit (b₀ ∷ []) B₂ q b =
    cong suc (syncs-lsplit [] B₂ q b)
  syncs-lsplit (b₀ ∷ b₁ ∷ B₁) B₂ q b =
    cong suc (syncs-lsplit (b₁ ∷ B₁) B₂ q b)

  lsplit-flags :
    ∀ B₁ B₂ q b {n : ℕ} (r c : 𝔽 n)
      (e₁ e₂ : SoupTerm.Tm n) →
    proj₂ (Translation.UB[ leftGroup B₁ q b B₂ ] r (e₁ , c , e₂)) ≡
    proj₂ (Translation.UB[ leftGroup′ B₁ q b B₂ ] r (e₁ , c , e₂))
  lsplit-flags B₁ B₂ q b r c e₁ e₂ =
    UB-flags-shape (leftGroup B₁ q b B₂) r c e₁ e₂
    ■ bindFlags-lsplit B₁ B₂ q b
    ■ sym (UB-flags-shape (leftGroup′ B₁ q b B₂) r c e₁ e₂)
...
U-lsplit :
  ...
  V.[] ; Context.[]
    ⊢ₚ lsplitSource B₁ B₂ B q b₁ s E P →
  SoupImage (lsplitSource B₁ B₂ B q b₁ s E P) C →
  Σ[ C′ ∈ Soup.Config n m ]
    (C SoupReduction.─→ₚ C′) ×
    SoupImage (lsplitTarget B₁ B₂ B q b₁ s E P) C′
U-lsplit {B₁ = B₁} {B₂ = B₂} {B = B} {q = q} {b₁ = b₁}
  {s = s} {E = E} {P = P} {n = n} {m = m} {C = C} ⊢P image
  with lsplit-confine All.[]
    {B₁ = B₁} {B₂ = B₂} {B = B} {q = q} {b₁ = b₁}
    {s = s} {E = E} {P = P} ⊢P
... | k , ρ⁻ , ρ⁻-skip , E₀ , Eeq , P₀ , Peq =
  C′ ,
  SoupReduction.RUS-LSplit j i zero F selected-channel selected-thread ,
  record
  ...
  τ₀ : Translation.Env (sum (leftGroup′ B₁ q b₁ B₂) + sum B + 0)
         (2 *ℕ Translation.channelCount TargetProc)
  τ₀ = targetEnv {B₁ = B₁} {B₂ = B₂} {B = B} {q = q} {b₁ = b₁} 0F

  τ : Translation.Env (sum (leftGroup′ B₁ q b₁ B₂) + sum B + 0) (2 *ℕ n)
  ...
  live-channel′ zero =
    ...
    ■ cong₂ (λ fs₁ fs₂ → (true , fs₁ , fs₂))
        (UB-flags-shape (leftGroup B₁ q b₁ B₂)
           (Soup.leftEnd {n = Translation.channelCount SourceProc} 0F)
           (Soup.leftEnd {n = Translation.channelCount SourceProc} 0F)
           (SoupTerm.K {n = 2 *ℕ Translation.channelCount SourceProc} Source.`unit)
           (SoupTerm.K {n = 2 *ℕ Translation.channelCount SourceProc} Source.`unit)
         ■ bindFlags-lsplit B₁ B₂ q b₁
         ■ sym (UB-flags-shape (leftGroup′ B₁ q b₁ B₂)
           (Soup.leftEnd {n = Translation.channelCount TargetProc} 0F)
           (Soup.leftEnd {n = Translation.channelCount TargetProc} 0F)
           (SoupTerm.K {n = 2 *ℕ Translation.channelCount TargetProc} Source.`unit)
           (SoupTerm.K {n = 2 *ℕ Translation.channelCount TargetProc} Source.`unit)))
        (UB-flags-cong B
          ...
          ((SoupTerm.K {n = 2 *ℕ Translation.channelCount TargetProc} Source.`unit) ,
           ...
           (SoupTerm.K {n = 2 *ℕ Translation.channelCount TargetProc} Source.`unit)))
  live-channel′ (suc l) =
    live-channel image (suc (Fin.cast (channelCount-⋯ₚ P 𝐒.lwk) l))
    ■ cong
        (λ channels →
          lookup channels (Fin.cast (channelCount-⋯ₚ P 𝐒.lwk) l))
        (flatten-channels-shape P
          (V.tabulate (λ x → suc x)) σ₀)
    ■ cong
        (λ channels →
          lookup channels (Fin.cast (channelCount-⋯ₚ P 𝐒.lwk) l))
        (sym (channelShape-⋯ₚ P 𝐒.lwk))
    ■ VecP.lookup-cast (channelCount-⋯ₚ P 𝐒.lwk)
        (channelShape (P Typed.⋯ₚ 𝐒.lwk)) l
    ■ cong (λ channels → lookup channels l)
        (sym (flatten-channels-shape (P Typed.⋯ₚ 𝐒.lwk)
  ...
  selected-thread :
    ...
    plug*ᵘ F (SoupTerm.K (Source.`lsplit s) SoupTerm.·¹ triple)
  selected-thread =
    live-thread image 0F
    ■ sym (T[_]-renEnv (plug*ˢ E
          (Source.K (Source.`lsplit s) Source.·¹ (Source.` x))) σ₀ ρ)
    ■ T[_]-plugᶠ* E Vσ
    ■ cong (λ z → plug*ᵘ F (SoupTerm.K (Source.`lsplit s) SoupTerm.·¹ z))
        x-triple

  target-left₀ :
    τ (𝐒.atk {q + suc (suc b₁)} {0} (q ↑ʳ 0F)) ≡
    chanTriple e₁ (Soup.endpoint i zero) SoupTerm.*
  target-left₀ =
    cong (SoupTerm._⋯ᵣ endpointEmbedding′)
      (cong τ₀ (atk-blockAt B₁ B₂ B (q + suc (suc b₁)) (q ↑ʳ 0F))
       ■ ++ₛ-lookupˡ
          (proj₁ (Translation.UB[ leftGroup′ B₁ q b₁ B₂ ]
            (Soup.leftEnd 0F)
            (SoupTerm.* , Soup.leftEnd 0F , SoupTerm.*))
           Translation.++ₛ
           proj₁ (Translation.UB[ B ] (Soup.rightEnd 0F)
            (SoupTerm.* , Soup.rightEnd 0F , SoupTerm.*)))
          (λ ())
          (blockAt B₁ B₂ (q + suc (suc b₁)) (q ↑ʳ 0F) ↑ˡ sum B)
       ■ ++ₛ-lookupˡ
          (proj₁ (Translation.UB[ leftGroup′ B₁ q b₁ B₂ ]
            (Soup.leftEnd 0F)
            (SoupTerm.* , Soup.leftEnd 0F , SoupTerm.*)))
          (proj₁ (Translation.UB[ B ] (Soup.rightEnd 0F)
            (SoupTerm.* , Soup.rightEnd 0F , SoupTerm.*)))
          (blockAt B₁ B₂ (q + suc (suc b₁)) (q ↑ʳ 0F))
       ■ proj₁ (proj₂ (proj₂ (proj₂ targetSplitShape))))
    ■ cong₂ (λ a j → chanTriple a j SoupTerm.*)
        (sym (proj₁ split-witness-source-target))
        (endpoint-respects-channel′ 0F zero)

  target-left₁ :
    τ (𝐒.atk {q + suc (suc b₁)} {0} (q ↑ʳ 1F)) ≡
    chanTriple SoupTerm.* (Soup.endpoint i zero) e₂
  target-left₁ =
    cong (SoupTerm._⋯ᵣ endpointEmbedding′)
      (cong τ₀ (atk-blockAt B₁ B₂ B (q + suc (suc b₁)) (q ↑ʳ 1F))
       ■ ++ₛ-lookupˡ
          (proj₁ (Translation.UB[ leftGroup′ B₁ q b₁ B₂ ]
            (Soup.leftEnd 0F)
            (SoupTerm.* , Soup.leftEnd 0F , SoupTerm.*))
           ...
           proj₁ (Translation.UB[ B ] (Soup.rightEnd 0F)
            (SoupTerm.* , Soup.rightEnd 0F , SoupTerm.*)))
          (λ ())
          (blockAt B₁ B₂ (q + suc (suc b₁)) (q ↑ʳ 1F) ↑ˡ sum B)
       ■ ++ₛ-lookupˡ
          (proj₁ (Translation.UB[ leftGroup′ B₁ q b₁ B₂ ]
            (Soup.leftEnd 0F)
            (SoupTerm.* , Soup.leftEnd 0F , SoupTerm.*)))
          ...
          (blockAt B₁ B₂ (q + suc (suc b₁)) (q ↑ʳ 1F))
       ■ proj₂ (proj₂ (proj₂ (proj₂ targetSplitShape))))
    ■ cong₂ (λ j z → chanTriple SoupTerm.* j z)
        (endpoint-respects-channel′ 0F zero)
        (sym (proj₂ split-witness-source-target))
  ...
  target-body :
    ...
    ≡
    Translation.T[
      (Source.` (𝐒.atk {q + suc (suc b₁)} {0} (q ↑ʳ 0F))) Source.⊗
      (Source.` (𝐒.atk {q + suc (suc b₁)} {0} (q ↑ʳ 1F)))
    ] τ
  target-body = cong₂ SoupTerm._⊗_ (sym target-left₀) (sym target-left₁)
  ...
  frame-lwk :
    ...
    plug*ᵘ F t ≡
    plug*ᵘ
      (Tᶠ*[ E SourceReduction.⋯ᶠ* 𝐒.lwk ] {σ = τ} Vτ)
      t
  frame-lwk t =
    cong (λ E′ → plug*ᵘ (Tᶠ*[ E′ ] {σ = σ} Vσ) t) Eeq
    ■ Tᶠ*-plug-ren-ren-coh E₀ ρ⁻ 𝐒.lwk σ τ Vσ Vτ
        (λ y → source-target-lwk (ρ⁻ y) (ρ⁻-skip y)) t
    ■ cong
        (λ E′ →
          plug*ᵘ
            (Tᶠ*[ E′ SourceReduction.⋯ᶠ* 𝐒.lwk ] {σ = τ} Vτ) t)
        (sym Eeq)
  ...
  parent-live =
    ...
    ■ sym (T[_]-plugᶠ*
        (E SourceReduction.⋯ᶠ* 𝐒.lwk)
        Vτ)
    ■ T[_]-renEnv
        (plug*ˢ (E SourceReduction.⋯ᶠ* 𝐒.lwk)
          ((Source.` (𝐒.atk {q + suc (suc b₁)} {0} (q ↑ʳ 0F))) Source.⊗
           (Source.` (𝐒.atk {q + suc (suc b₁)} {0} (q ↑ʳ 1F)))))
        τ₀ endpointEmbedding′
