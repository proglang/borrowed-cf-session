...
data Frame n : Set where
  ...
  □,⟨_⟩_ : (d : Dir) (e₂ : Tm n) → Frame n
  _,⟨_⟩□ : {e₁ : Tm n} (V₁ : Value e₁) (d : Dir) → Frame n
  □;_ : (e₂ : Tm n) → Frame n
...
module _ (Γ-S : ChanCx Γ) where
  ...
  preservation′ (T-LetUnit p/s e₁ e₂) E-Seq =
    ...
    in
    T-Weaken ≤ϵ-refl γ≼ e₂
  preservation′ (T-LetPair {d = d} {T₂ = T₂} p/s {γ₁} {γ₂} e e′) (E-PairElim V₁ V₂) with refl ← ⊢pair-dir-uniq e =
  ...
  ... | T-Weaken ϵ≤ γ≤ e′ = T-Weaken ϵ≤ γ≤ (preservation e′ E)
  preservation e E@(E-Ctx (□,⟨ d ⟩ _) E₁) with e
  ... | T-Pair p/s e₁ e₂ seq⇒p = T-Pair p/s (preservation e₁ E₁) e₂ seq⇒p
  ... | T-Weaken ϵ≤ γ≤ e′ = T-Weaken ϵ≤ γ≤ (preservation e′ E)
  preservation e E@(E-Ctx (V₁ ,⟨ d ⟩□) E₂) with e
...
module _ where
  ...
  data _─→ₚ_ {n} : Proc n → Proc n → Set where
    ...
    R-New : (E : Frame n) →
      ⟪ E [ K (`new s) · K `unit ] ⟫
        ─→ₚ
      ν (0 ∷ 1 ∷ [] , refl) (0 ∷ 1 ∷ [] , refl) ⟪ E ⋯ᶠ weaken* ⦃ Kᵣ ⦄ _ [ (` 0F) ,⟨ 𝟙 ⟩ (` 1F) ] ⟫
