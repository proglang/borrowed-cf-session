\PassOptionsToPackage{dvipsnames}{xcolor}
\documentclass[acmsmall,screen,review,anonymous,nonacm]{acmart}

\usepackage{mathpartir}
\usepackage{stmaryrd}
\usepackage{placeins}
\usepackage{mathtools}
\usepackage{cleveref}

\input{symbols}
\input{rules}

\title{Context-Free Session Types with Borrowing}
\date{August 28, 2025}

\begin{document}
\maketitle

\begin{figure}[tp]
  \begin{itemize}
  \item prove subject reduction (start: April 24)
  \item sketch translation, prove simulation (start: April 24)
  \item target FreeST according to I\&C paper; encode existentials by
    universals
  \item maybe: bisimulation
  \item later: add recursive types and recursion to source language
  \item later: add explicit polymorphism as in FreeST
  \end{itemize}
  \caption{The Plan}
  \label{fig:the-plan}
\end{figure}


The next couple of lemmas are needed to prove that
expression reduction preserves typing.

\begin{lemma}
  If $\HasType[\Eff] {\Ctx} {\V} {\T}$
  then $\Eff = \EffPure$.
\end{lemma}
\begin{proof}
  By induction on the derivation of $\HasType[\Eff] {\Ctx} {\V} {\T}$.
\end{proof}

\begin{lemma}[Substitution]\
  \begin{itemize}
    \item If $\HasType[\Eff[1]] {\Ctx[1]} {\V} {\T[1]}$ and
    $\HasType[\Eff[2]] {\CtxVar x{\T[1]} \CtxPar \Ctx[2]} {\E} {\T[2]}$
    then 
    $\HasType[\Eff[2]] {\Ctx[1] \CtxPar \Ctx[2]} {\Subs\E\V x} {\T[2]}$
    \item If $\HasType[\Eff[1]] {\Ctx[1]} {\V} {\T[1]}$ and
    $\HasType[\Eff[2]] {\CtxVar x{\T[1]} \CtxSeq \Ctx[2]} {\E} {\T[2]}$
    then 
    $\HasType[\Eff[2]] {\Ctx[1] \CtxSeq \Ctx[2]} {\Subs\E\V x} {\T[2]}$
    \item If $\HasType[\Eff[1]] {\Ctx[1]} {\V} {\T[1]}$
    and
    $\HasType[\Eff[1]]
    {\CtxPattern{\CtxVar x{\T[1]} \CtxPar \Ctx[2]}}
    {\E}
    {\T[2]}$ then 
    $\HasType[\Eff[2]] {\CtxPattern{\Ctx[1] \CtxPar \Ctx[2]}} {\Subs\E\V x} {\T[2]}$
  \item If $\HasType[\Eff[1]] {\Ctx[1]} {\V} {\T[1]}$
    and
    $\HasType[\Eff[2]]
    {\CtxPattern{\CtxVar x{\T[1]} \CtxSeq \Ctx[2]}}
    {\E}
    {\T[2]}$ then 
    $\HasType[\Eff[2]] {\CtxPattern{\Ctx[1] \CtxSeq \Ctx[2]}} {\Subs\E\V x} {\T[2]}$
  \end{itemize}
\end{lemma}
\begin{lemma}[Substitution]
  \label{lem:substitution}
  If 
  $\HasType[\EffPure] {\Ctx[0]} {\E[0]} {\T[0]}$
  and
  ($\MobMobile (\T[0])$ implies $\MobMobile (\Ctx[0])$)
  and
  $\HasType[\Eff] {\CtxPattern[0]{\CtxVar y{\T[0]}}} \E \T$
    implies
    $\HasType[\Eff] {\CtxPattern[0]{{\Ctx[0]}}} {\Subs\E{\E[0]} y} {\T}$,
    for disjoint $\Ctx[0]$ and $\CtxPatternRaw_0$.
  \end{lemma}
  \pt{somehow we need to make sure that $\Ctx[0]$ is independent so
  that it can safely be moved around in the context.}
\begin{proof}
  By rule induction on the derivation of $\HasType[\Eff]
  {\CtxPattern[0]{\CtxVar x{\T[0]}}} \E \T$.

  \textbf{Case} $\RuleTConst$. n.a.

  \textbf{Case} $\RuleTVar$. Immediate for $x=y$ and empty $\CtxPattern[0]{}$.

  \textbf{Case} $\RuleTAbsLin$. Here $x\ne y$; by variable convention, assume
  $x \notin dom (\Ctx[0])$;
  we have $\Ctx = \CtxPattern[0]{{\T[0]}}$;
  if $\Mob = \MobMobile$, then $\MobMobile (\T[0])$;
  by induction,
  $\HasType[\Eff]
  {\CtxPattern[0]{{\Ctx[0]}} \CtxPar \CtxVar\EVar{\T[1]}}
  {\Subs\E{\E[0]} y} {\T[2]}$;   if $\Mob = \MobMobile$, then, by assumption, $\MobMobile
  (\Ctx[0])$ which implies $\MobMobile (\CtxPattern[0]{{\Ctx[0]}})$; conclude
  by \TirName{T-AbsLin}.

  \textbf{Case} \TirName{T-AbsLeft} and \TirName{T-AbsRight} are
  analogous.

  \textbf{Case} $\RuleTAppLin$. \\
  Either $y\in \E[1]$ or $y \in \E[2]$.
  If $y\in \E[1]$, then $\Ctx[1]$ has the form $\CtxPattern[0]{\CtxVar y
    {\T[0]}}$ and induction yields
  $\HasType[\Eff[1]] {\CtxPattern[0]{\Ctx[0]}} {\Subs{\E[1]}{\E[0]}y}
  {(\TArr{\Mob}{\DirUnord,{\Eff[3]}}{\T[1]}{\T[2]})}$.
  Conclude by applying \TirName{TApp-Lin}. 

  If $y\in \E[2]$, argue analogously.

  \textbf{Case} \TirName{T-AppLeft} and \TirName{T-AppRight} are
  analogous.

  \textbf{Case} \TirName{T-PairUnord} and \TirName{T-PairOrd} are analogous.

  \textbf{Case} $\RuleTLet$.

  Either $y\in\E[1]$ or $x\ne y$ and $y\in\E[2]$.

  If $y\in\E[1]$, then $\Ctx$ has the form $\CtxPattern[1]{\CtxVar y {\T[0]}}$
  and $\CtxPattern[0]{} = \CtxPattern{\CtxPattern[1]{}}$. By induction,
  $\HasType[\Eff[1]] {\CtxPattern[1]{\Ctx[0]}} {\E[1]} {\T[1]}$ and applying
  \TirName{T-Let} yields a typing in context $\CtxPattern[0]{\Ctx[0]}$.

  If $x \ne y \in \E[2]$, then $y$ must appear in $\CtxPatternRaw$, so
  that $\CtxPattern{\Ctx} = \CtxPattern[0]{\CtxVar y {\T[0]}}[{\Ctx}]$, for
  some 2-hole pattern $\CtxPatternRaw_0$. By induction using the
  pattern $\CtxPattern[0]{\CtxVar y {\T[0]}}[{\CtxVar x {\T[1]}}]$, we
  obtain a judgment in context $\CtxPattern[0]{\Ctx[0]}[{\CtxVar x {\T[1]}}]$
 and applying \TirName{T-Let}
  we obtain a typing in context $\CtxPattern[0]{\Ctx[0]}[{\Ctx}]$. The
  $\IsLeftPat$ property is not affected as it refers to the position
  of $x$ in the context.

  \textbf{Case} $\RuleTSeq$.

  Either $y \in \E[1]$ or $y \in \E[2]$.

  If $y \in \E[1]$, then the argument is analogous to \TirName{T-Let}.

  If $y \in \E[2]$, then $y$ must appear in $\CtxPatternRaw$, so
  that $\CtxPattern{\Ctx} = \CtxPattern[0]{\CtxVar y {\T[0]}}[{\Ctx}]$, for
  some 2-hole pattern $\CtxPatternRaw_0$. By induction using the
  pattern $\CtxPattern[0]{\CtxVar y {\T[0]}}[\CtxEmpty]$, we
  obtain a judgment in context $\CtxPattern[0]{\Ctx[0]}[\CtxEmpty]$
 and applying \TirName{T-Seq}
 we obtain a typing in context $\CtxPattern[0]{\Ctx[0]}[{\Ctx}]$.

 \textbf{Case} \TirName{T-Seq} and \TirName{T-LetOrd} are analogous
 to \TirName{T-Let} and \TirName{T-Seq}.

 \textbf{Case} \TirName{T-Inj}. Immediate by induction.

 \textbf{Case} \TirName{T-CaseSum}. Analogous to \TirName{T-Let}.

 \textbf{Case} \TirName{T-Weaken}.

 \textbf{Case} \TirName{T-Effect}.

 We are given $\HasType[\Eff[2]] {\CtxPattern[0]{\CtxVar y {\T[0]}}} \E \T$,
 that is, $\Ctx[2] = \CtxPattern[0]{\CtxVar y {\T[0]}}$.
 We have to show that $\Ctx[2] \preccurlyeq \Ctx[1]$ implies the
 existence of some $\CtxPatternRaw_1$ such that $\Ctx[1] =
 \CtxPattern[1]{\CtxVar y {\T[0]}}$. This result follows by induction
 on the number of applications of the rewrite rule for weakening.
 In the same way we can show that
 $\CtxPattern[0]{\CtxVar y {\T[0]}}\preccurlyeq \CtxPattern[1]{\CtxVar y {\T[0]}}$
 implies
 $\CtxPattern[0]{\Ctx[0]}\preccurlyeq
 \CtxPattern[1]{\Ctx[0]}$.
 Conclude by applying rule \TirName{T-WeakCE}.
\end{proof}

\begin{lemma}[Expression reduction preserves typing]
  If $\E[1] \EReduce \E[2]$ and
  $\HasType[\Eff[1]]
      {\Ctx}
      {\E[1]}
      {\T}$
  then 
  $\HasType[\Eff[2]]
      {\Ctx}
      {\E[2]}
      {\T}$
  for some $\Eff[2] \leq \Eff[1]$.
\end{lemma}

The following is an auxiliary lemma to prove that 
process reduction preserves typing.

\begin{lemma}
    $\HasType[\Eff[1]]
      {\Ctx[1] \CtxPar \Ctx[2]}
      {\ECapp\E}
      {\T[1]}$
    if and only if
    $\HasType[\Eff[2]]
      {\Ctx[1]}
      {\E}
      {\T[2]}$
    and 
    $\HasType[\Eff[1]]
      {\CtxVar x{\T[2]} \CtxPar \Ctx[2]}
      {\ECapp x}
      {\T[1]}$
\end{lemma}

\begin{lemma}
  \label{lemma:inversion-evaluation-context}
  If $\HasType[\Eff]{\Ctx}{\ECapp\E}{\T}$, then there exists $\Ctx*$,
  $\T*$, and $\Eff*$ such that $\HasType[\Eff*]{\Ctx*}{\E}{\T*}$.
\end{lemma}
\begin{proof}
  By induction on $\EC$.
\end{proof}

\begin{lemma}
  \label{lemma:inversion-process-evaluation-context}
  If $\ProcHasType{\Ctx[0]} {\FCapp{\E}}$, then there exists $\Ctx$,
  $\T$, and $\Eff$ such that $\HasType[\Eff]{\Ctx}{\E}{\T}$.
\end{lemma}
\begin{proof}
  It must be that $\FC = \PExp \EC$, for some evaluation context $\EC$.
  By inversion of the process typing,
  $\HasType[\EffImpure]{\Ctx[0]}{\ECapp{\E}}{\BUnit}$.
  By Lemma~\ref{lemma:inversion-evaluation-context}, there is  $\Ctx$,
  $\T$, and $\Eff$ such that $\HasType[\Eff]{\Ctx}{\E}{\T}$.
\end{proof}

\clearpage
\begin{lemma}[Process reduction preserves typing]
  If $\Proc[1] \PReduce \Proc[2]$ and
  $\ProcHasType {\Ctx} {\Proc[1]}$
  then 
  $\ProcHasType {\Ctx} {\Proc[2]}$.
\end{lemma}
\begin{proof}
  \textbf{Case} \TirName{R-Exp}.

  \textbf{Case} $\left\{\RuleProcRedNew\right.$.\\
  As $\ProcHasType\Ctx {\FCapp{\EApp \CNew \CUnit}}$, there must be
  some $\Ctx*$ such that
  $\HasType[\Eff]{\Ctx*}{\EApp \CNew \CUnit}{\T}$ (by Lemma~\ref{lemma:inversion-process-evaluation-context}). \\
  By inversion, we obtain $\Ctx* = \CtxEmpty$ and $\T =
  \TPairUnord{(\SSeq{\SAcq}{\S})}{(\SSeq{\SAcq}{\Dual~\S})}$. \\
  Let $\Ctx** = \CtxVar x{(\SSeq{\SAcq}{\S})}  \CtxPar \CtxVar y {\SSeq{\SAcq}{\Dual~\S}}$. \\
  We have
  $\HasType{\Ctx**}{\EPair
    xy}{\TPairUnord{(\SSeq{\SAcq}{\S})}{(\SSeq{\SAcq}{\Dual~\S})}}{\EffPure}$.
  \\
  By lemma XXX, we have
  $\ProcHasType{\Ctx \CtxPar \Ctx**}{\FCapp{\EPair xy}}$.
  \\
  Let $\Ctx[1] =\CtxVar x{(\SSeq{\SAcq}{\S})}$ and $\Ctx[2] = \CtxVar
  y {\SSeq{\SAcq}{\Dual~\S}}$ so that $\Ctx** =
  \Ctx[1] \CtxPar \Ctx[2]$.
  \\
  Clearly, $\S \vdash {\BLeadSep{x}} \leadsto \Ctx[1]$ and
  $\Dual~\S \vdash {\BLeadSep{y}} \leadsto \Ctx[2]$ hold.
  \\
  Conclude with rule \TirName{TP-Res} and obtain
  $\ProcHasType {\Ctx}
  {\PNew {\BLeadSep{x}}{\BLeadSep{y}}{\FCapp{\EPair xy}}}$.

  

  \textbf{Case} $\left\{ \RuleProcRedFork \right.$.\\
  As $\ProcHasType\Ctx {\FCapp{\EApp \CFork \V}}$, there must be some
  $\Ctx*$ such that
  $\HasType[\Eff]{\Ctx*}{\EApp \CFork \V}{\T}$ (by
  Lemma~\ref{lemma:inversion-process-evaluation-context}). \\
  By inversion, we obtain
  $\HasType[\EffPure]{\Ctx*}\V{\TArr{\MobMobile}{\DirUnord,\EffImpure}\BUnit\BUnit}$,
  so that $\T = \BUnit$ and $\Eff = \EffImpure$ and $\Ctx =
  \Ctx* \CtxPar \Ctx**$ (because the function is $\DirUnord$ its
  application must do a parallel split), for some $\Ctx**$.
  \\
  NEED A LEMMA to conclude $\ProcHasType{\Ctx**}{\FCapp{\CUnit}}$.
  \\
  Clearly, $\ProcHasType{\Ctx*}{\PExp {\EApp\V\CUnit}}$, so that
  $\ProcHasType{\Ctx* \CtxPar \Ctx**}{\PPar{\FCapp{\CUnit}}{\PExp {\EApp\V\CUnit}}}$.

  Case \TirName{R-Com}.

  Case \TirName{R-RSplit}.

  Case \TirName{R-LSplit}.

  Case \TirName{R-Drop}.

  Case \TirName{R-Acquire}.

  Case \TirName{R-Close}.

  Case \TirName{R-Par}.

  Case \TirName{R-Bind}.

  Case \TirName{R-Struct}.
\end{proof}

\input{sec/expressions}
\clearpage
\input{sec/types}
\clearpage
\input{sec/algorithmic}
\clearpage
\input{sec/translation}

\end{document}

%%% Local Variables:
%%% mode: latex
%%% TeX-master: t
%%% End:
