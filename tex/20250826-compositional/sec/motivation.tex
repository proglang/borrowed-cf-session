...
\lstinline{drop} on the channel, which synchronizes with the
\lstinline{acquire} on the suffix of the channel.
...
signalling primitives for implementing \lstinline{acquire}.

Specifically, a channel is represented by a triple
$in \otimes c \otimes out$, where $c$ is a primitive channel and $in$
and $out$ are optional synchronization channels. These synchronization
channels carry only a single bit.

Allocating a channel creates a primitive channel and one
synchronization channel for each channel end.
