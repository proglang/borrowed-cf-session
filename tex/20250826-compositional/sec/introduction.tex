...
between local borrows that remain in the same thread and remote
borrows that are passed to other threads. This uniformity leads to
...
different: once a borrow crosses a thread boundary, evaluation order
...
\item We distinguish local borrowing from remote borrowing. Local
  borrows remain in one thread and can be ordered by evaluation and
  typing alone; remote borrows cross thread boundaries and therefore
...
\item We define a lower-level run-time calculus with explicit channel
  ...
  \thecalculus threads to this calculus.
