...
preservationₚ {Γ = Γ} Γ-S p (R-Com {b₁ = b₁}{B₁ = B₁}{b₂ = b₂}{B₂ = B₂} {E₁ = E₁}{E₂} Vv)
  ...
  with (T₁ ∷ Γ₁) , (T₂ ∷ Γ₂) , _ , pl , N , ⊢B₁ , ⊢B₂ , C₁ , C₂ , p′ ← inv-ν p
  ...
  with ⟨ eq-c₁ ⟩ , `c₁≤ ← inv-` ⊢c₁
  with eq-c₂′ , `c₂≤ ← inv-` ⊢c₂
  with ⟨ eq-c₂ ⟩ ← subst (_ ≃_) (V.lookup-++ˡ (Γ₁ ⸴* T₂ ⸴ Γ₂) Γ (b₁ + sum B₁ ↑ʳ 0F) ■ V.lookup-++ʳ Γ₁ (T₂ ⸴ Γ₂) 0F) eq-c₂′
  = let ⊢wkρ = ⊢wkₚ Γ₁ Γ₂ Γ {T₁} {T₂} in
    let wkρ-inj = wkₚ-inj (b₁ + sum B₁) (b₂ + sum B₂) in
    let γ′ , ≤γ , q′ = q ⊢⋯ₚ⁻¹ ⊢wkρ / wkₚ-inj (b₁ + sum B₁) (b₂ + sum B₂) _ in
    let 𝒫₁ , 𝒫₁≤ , ⊢E₁′ = ⊢E₁ ⊢⋯ᶠ*⁻¹ ⊢wkρ / wkρ-inj _ in
    let 𝒫₂ , 𝒫₂≤ , ⊢E₂′ = ⊢E₂ ⊢⋯ᶠ*⁻¹ ⊢wkρ / wkρ-inj _ in
    let α-v′ , α-v′≤ , ⊢v′ = ⊢v ⊢⋯⁻¹ ⊢wkρ / wkρ-inj _ in
    let s₁′ , ‼T;s₁′≃ , C₁′ = bindCtx-inv-msg (≃-trans (≃-trans (≃-sym eq-c₁) c₁≃) (≃-sym ‼T≃)) C₁ in
    let s₂′ , ⁇T;s₂′≃ , C₂′ = bindCtx-inv-msg (≃-trans (≃-sym eq-c₂) (≃-sym ⁇T≃)) C₂ in
      TP-Res {!!} pl ⊢B₁ ⊢B₂ (bindCtx-≃ {!‼T;s₁′≃`!} C₁′) {!!}
      $ TP-Weaken {!!}
      $ TP-Par {!!}
               q′
  {-
