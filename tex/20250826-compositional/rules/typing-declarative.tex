% Declarative Expression Typing

\def\RuleNamePrefix{T-}
\def\RuleCmdPrefix{RuleT}

% T-Const
\DefRule {Const} {
  \HasCType
    \C
    \T
}{
  \HasType[\EffPure]
    \CtxEmpty
    \C
    \T
}

% T-Var
\DefRule {Var} {} {
  \HasType[\EffPure]
    {\CtxVar \EVar\T}
    \EVar
    \T
}

% T-AbsUnr
\DefRule {AbsUnr} {
  \HasType
    {
        \CtxVar f {\TArr{\MobMobile}{\DirNone,\Eff}{\T[1]}{\T[2]}}
          \CtxPar \CtxVar \EVar{\T[1]}
          \CtxPar \Ctx
    }
    {\E}
    {\T[2]} \\
  \Unr\Ctx
}{
  \HasType[\EffPure]
    \Ctx
    {\EMu {\EAbs\EVar\E}}
    {\TArr{\MobMobile}{\DirNone,\Eff}{\T[1]}{\T[2]}}
}

% T-AbsLin
\DefRule {AbsLin} {
  \HasType
    {\CtxVar\EVar{\T[1]} \CtxPar \Ctx}
    {\E}
    {\T[2]} \\
  \Mob (\Ctx)
}{
  \HasType[\EffPure]
    \Ctx
    {\EAbs\EVar\E}
    {\TArr{\Mob}{\DirUnord,\Eff}{\T[1]}{\T[2]}}
}

% T-AbsLeft
\DefRule {AbsLeft} {
  \HasType
    {\CtxVar{\EVar}{\T[1]} \CtxSeq \Ctx}
    {\E}
    {\T[2]} \\
  \Mob (\Ctx)
}{
  \HasType[\EffPure]
    \Ctx
    {\EAbs\EVar\E}
    {\TArr{\Mob}{\DirLeft,\Eff}{\T[1]}{\T[2]}}
}

% T-AbsRight
\DefRule {AbsRight} {
  \HasType
    {\Ctx \CtxSeq \CtxVar{\EVar}{\T[1]}}
    {\E}
    {\T[2]} \\
  \Mob (\Ctx)
}{
  \HasType[\EffPure]
    \Ctx
    {\EAbs\EVar\E}
    {\TArr{\Mob}{\DirRight,\Eff}{\T[1]}{\T[2]}}
}

% T-AppUnr
\DefRule {AppUnr} {
  \HasType
    {\Ctx[1]}
    {\E[1]}
    {\TArr{\Mob}{\DirNone,\Eff}{\T[1]}{\T[2]}} \\
  \HasType
    {\Ctx[2]}
    {\E[2]}
    {\T[1]}
}{
  \HasType
    {\Ctx[1] \CtxSeq \Ctx[2]}
    {\E[1] \E[2]}
    {\T[2]}
}

% T-AppLine
\DefRule {AppLin} {
  \HasType
    {\Ctx[1]}
    {\E[1]}
    {\TArr{\Mob}{\DirUnord,\Eff}{\T[1]}{\T[2]}} \\
  \HasType
    {\Ctx[2]}
    {\E[2]}
    {\T[1]}
}{
  \HasType
    {\Ctx[1] \CtxPar \Ctx[2]}
    {\E[1] \E[2]}
    {\T[2]}
}

% T-AppLeft
\DefRule {AppLeft} {
  \HasType[\EffPure]
    {\Ctx[1]}
    {\E[1]}
    {\TArr{\Mob}{\DirLeft,\Eff}{\T[1]}{\T[2]}}
  \\
  \HasType
    {\Ctx[2]}
    {\E[2]}
    {\T[1]}
}{
  \HasType
    {\Ctx[2] \CtxSeq \Ctx[1]}
    {\E[1] \E[2]}
    {\T[2]}
}

% T-AppRight
\DefRule {AppRight} {
  \HasType
    {\Ctx[1]}
    {\E[1]}
    {\TArr{\Mob}{\DirRight,\Eff}{\T[1]}{\T[2]}}
    \\
  \HasType[\EffPure]
    {\Ctx[2]}
    {\E[2]}
    {\T[1]}
}{
  \HasType
    {\Ctx[1] \CtxSeq  \Ctx[2]}
    {\E[1] \E[2]}
    {\T[2]}
}

% T-PairUnord
\DefRule {PairUnord} {
  \HasType
    {\Ctx[1]}
    {\E[1]}
    {\T[1]} \\
  \HasType
    {\Ctx[2]}
    {\E[2]}
    {\T[2]}
}{
  \HasType
    {\Ctx[1] \CtxPar  \Ctx[2]}
    {\EPair{\E[1]}{\E[2]}}
    {\TPairUnord{\T[1]}{\T[2]}}
}

% T-PairOrd
\DefRule {PairOrd} {
  % \Unr{\Ctx[1]} \vee \Unr {\T[1]} \vee \Eff[2] = \EffPure \\
  \HasType
    {\Ctx[1]}
    {\E[1]}
    {\T[1]} \\
  \HasType[\EffPure]
    {\Ctx[2]}
    {\E[2]}
    {\T[2]}
}{
  \HasType
    {\Ctx[1] \CtxSeq  \Ctx[2]}
    {\EPair{\E[1]}{\E[2]}}
    {\TPairOrd{\T[1]}{\T[2]}}
}

% T-LetUnit
\DefRule {LetUnit} {
  \HasType
    {\Ctx[1]}
    {\E[1]}
    {\BUnit} \\
  \HasType
    {\Ctx[2]}
    {\E[2]}
    {\T}
}{
  \HasType
    {\CtxPat 1 2}
    {\ESeq{\E[1]}{\E[2]}}
    {\T}
}

% T-Let
\DefRule {Let} {
  \HasType
    {\Ctx[1]}
    {\E[1]}
    {\T[1]} \\
  \HasType
    {\CtxVar x {{\T[1]}} \CtxPatOp \Ctx[2]}
    {\E[2]}
    {\T[2]}
}{
  \HasType
    {\CtxPat 1 2}
    {\ELet x{\E[1]}{\E[2]}}
    {\T[2]}
}

% T-LetUnord
\DefRule {LetUnord} {
  \HasType
    {\Ctx[1]}
    {\E[1]}
    {\TPairUnord {\T[1]}{\T[2]}} \\\\
  \HasType
    {\CtxVar x{\T[1]} \CtxPar \CtxVar y{\T[2]} \CtxPatOp \Ctx[2]}
    {\E[2]}
    {\T[3]}
}{
  \HasType
    {\CtxPat 1 2}
    {\ELetPair xy{\E[1]}{\E[2]}}
    {\T[3]}
}

% T-LetOrd
\DefRule {LetOrd} {
  \HasType
    {\Ctx[1]}
    {\E[1]}
    {\TPairOrd {\T[1]}{\T[2]}} \\\\
  \HasType
    {\CtxVar x{\T[1]} \CtxSeq \CtxVar y{\T[2]} \CtxPatOp \Ctx[2]}
    {\E[2]}
    {\T[3]}
}{
  \HasType
    {\CtxPat 1 2}
    {\ELetPair xy{\E[1]}{\E[2]}}
    {\T[3]}
}

% T-Inj
\DefRule {Inj} {
  \HasType
    {\Ctx}
    {\E}
    {\T[i]}
}{
  \HasType
    {\Ctx}
    {\EInj{i} \E}
    {\TSum{\T[1]}{\T[2]}}
}

% T-CaseSum
\DefRule {CaseSum} {
  \HasType
    {\Ctx[1]}
    {\E}
    {\TSum{\T[1]}{\T[2]}}
  \\
  \HasType
    {\CtxVar{\EVar[1]}{\T[1]} \CtxPatOp \Ctx[2]}
    {\E[1]}
    {\T}
  \\
  \HasType
    {\CtxVar{\EVar[2]}{\T[2]} \CtxPatOp \Ctx[2]}
    {\E[2]}
    {\T}
}{
  \HasType
    {\CtxPat 1 2}
    {\ECase\E{x_1}{\E[1]}{x_2}{\E[2]}}
    {\T}
}

% T-Weaken
\DefRule {Weaken} {
  \Ctx[1] \SubCtx \Ctx[2]
  \\
  \HasType
    {\Ctx[1]}
    {\E}
    {\T}
}{
  \HasType
    {\Ctx[2]}
    {\E}
    {\T}
}

% T-Effect
\DefRule {Effect} {
  \Eff[1] \le \Eff[2]
  \\
  \HasType[\Eff[1]]
    {\Ctx}
    {\E}
    {\T}
}{
  \HasType[\Eff[2]]
    {\Ctx}
    {\E}
    {\T}
}

% \newcommand*\RuleTMatch{
%   \inferrule[T-MatchLeft]{
%     \HasType[\Eff]
%       {\Ctx[1]}
%       {\E}
%       {\SBranch{\ell:\S[\ell]}{\ell\in L}}
%     \\
%     \HasType[\Eff[\ell]]
%       {\Ctx[2]}
%       {\E[\ell]}
%       {\TArr{\Mob}{\Eff[\ell]}{\S[\ell]}{\T}}
%   }{
%     \HasType[\Eff \EffJoin \EffJoin \{ \Eff[\ell] \}_{\ell\in L}]
%       {\Ctx[1] \CtxSeq \Ctx[2]}
%       {\EMatch{\E}{\ell:\E[\ell]}{\ell\in L}}
%       {\T}
%   }
% }
% 
% \newcommand*\RuleTMatchLin{
%   \inferrule[T-MatchLin]{
%     \HasType[\Eff]
%       {\Ctx[1]}
%       {\E}
%       {\SBranch{\ell:\S[\ell]}{\ell\in L}}
%     \\
%     \HasType[\Eff[\ell]]
%       {\Ctx[2]}
%       {\E[\ell]}
%       {\TArr{\Mob}{\Eff[\ell]}{\S[\ell]}{\T}}
%   }{
%     \HasType[\Eff \EffJoin \EffJoin\{ \Eff[\ell]\}_{\ell\in L}]
%       {\Ctx[1] \CtxPar \Ctx[2]}
%       {\EMatch{\E}{\ell:\E[\ell]}{\ell\in L}}
%       {\T}
%   }
% }

%%% Local

%%% Local Variables:
%%% mode: latex
%%% TeX-master: "../main"
%%% End:
