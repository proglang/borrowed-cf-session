module BorrowedCF.Simulation2.Linearity where

-- | LINEARITY SOUNDNESS for the channel-op simulation cases.
--   Originally built INSIDE BorrowedCF.Simulation2.Theorems (for the R-Discard
--   b=0 vacuity witness `discard-b0-vac`); hoisted here so that the confinement
--   lemmas in BorrowedCF.Simulation2.SplitConfine (which is imported by the case
