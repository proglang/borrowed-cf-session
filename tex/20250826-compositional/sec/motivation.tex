...
\lstinline{drop} on the channel, which sets a flag which unblocks a
...
\lstinline{Ret} only arise from a remote 
borrowing. The \emph{only} operation on a channel of type
...
demarcated by ``\lstinline{|}''. The \lstinline{drop} is only enabled
...
signaling primitives for implementing \lstinline{drop}/\lstinline{acquire}.
...
process carries one of three different values AA, BB, and CC.
EXPLAIN.
