module BorrowedCF.Processes.Congruence where
...
open import BorrowedCF.Terms.SubstitutionInversion using (brₛ)
...
private
  ...
  finC : ∀ a b {n} → ((𝐂.weaken* b 𝐂.·ₖ 𝐂.weaken* a) 𝐂.·ₖ 𝐂.assocSwapᵣ a b) ≗ (𝐂.weaken* a {n} 𝐂.·ₖ 𝐂.weaken* b)
...
νext-split {k} {n} Δ {Γ} {Fr} {Γγ₁} {Γγ₂} {γ} {γ₁′} Fr⊆ H1 H2 = ζ₂ , O1 , O2
  where
    ...
    RHSb≈ = let open ≈-Reasoning in begin
      (Fr ↓ base) ∥ (γwk ↓ base)
        ≈⟨ 𝐂.∥-cong (↓-empty Fr base (uncurry x∈p⇒x∉∁p ∘ Π.map₁ Fr⊆ ∘ x∈p∩q⁻ _ _ ∘ proj₂)) ≈-refl ⟩
      [] ∥ (γwk ↓ base)  ≈⟨ ∥-unit₁ ⟩
    ...
    C : Γ ∶ ζ₁ ∥ ζ₂ ≼ γ
    C = ≼-⋯⁻¹ {ϕ = 𝐂.weaken* k} (𝐂.wk*-inj k) (𝐂.wk*-⇔ Δ)
          (subst (λ z → (Δ ⸴* Γ) ∶ z ≼ γwk) (sym (cong₂ _∥_ e₁ e₂)) H1b)

    D : Γ ∶ γ₁′ ≼ ζ₁
    D = ≼-⋯⁻¹ {ϕ = 𝐂.weaken* k} (𝐂.wk*-inj k) (𝐂.wk*-⇔ Δ)
          (subst₂ (λ a b → (Δ ⸴* Γ) ∶ a ≼ b)
                  (↓-identity-⊆ (γ₁′ 𝐂.⋯ᵣ 𝐂.weaken* k) {∁ (fresh n k)} (dom⋯wk*⊆∁fresh k γ₁′))
                  (sym e₁) (↓-mono-≼ {X = base} H2))

    O2 : Γ ∶ γ₁′ ∥ ζ₂ ≼ γ
    O2 = ≼-trans (≼-cong-∥ D (≼-refl ≈-refl)) C

    Γγ₁f-Unr : AllCx Unr (Δ ⸴* Γ) (Γγ₁ ↓ fresh n k)
    Γγ₁f-Unr = ↓-⊆ Γγ₁ (λ y∈ → x∉p⇒x∈∁p (λ y∈d → x∈∁p⇒x∉p (dom⋯wk*⊆∁fresh k γ₁′ y∈d) y∈))
                    (≼⇒extra-Unr H2)

    RHSf≈ : (Δ ⸴* Γ) ∶ (Fr ↓ fresh n k) ∥ (γwk ↓ fresh n k) ≈ Fr
    RHSf≈ = let open ≈-Reasoning in
      begin (Fr ↓ fresh n k) ∥ (γwk ↓ fresh n k)
              ≈⟨ 𝐂.∥-cong (≈-reflexive (↓-identity-⊆ Fr Fr⊆)) ≈-refl ⟩
            Fr ∥ (γwk ↓ fresh n k)
              ≈⟨ 𝐂.∥-cong ≈-refl (↓-empty γwk _ λ (x , x∈) →
                   let ∈dom , ∈fresh = x∈p∩q⁻ (dom γwk) (fresh n k) x∈ in
                   ⊥-elim (x∈p⇒x∉∁p ∈fresh (dom⋯wk*⊆∁fresh k γ ∈dom))) ⟩
            Fr ∥ []   ≈⟨ ∥-unit₂ ⟩
            Fr ∎

    F : (Δ ⸴* Γ) ∶ (Γγ₂ ↓ fresh n k) ≼ Fr
    F = ≼-trans (≼-trans (≼-refl (≈-sym ∥-unit₁)) (≼-cong-∥ (≼-∅ Γγ₁f-Unr) (≼-refl ≈-refl)))
                (≼-respʳ-≈ RHSf≈ (↓-mono-≼ {X = fresh n k} H1))

    O1 : (Δ ⸴* Γ) ∶ Γγ₂ ≼ Fr ∥ (ζ₂ 𝐂.⋯ᵣ 𝐂.weaken* k)
    O1 = ≼-trans (subst (λ z → (Δ ⸴* Γ) ∶ Γγ₂ ≼ (Γγ₂ ↓ fresh n k) ∥ z)
                        (sym e₂) (≼-↓∥↓∁ Γγ₂ (fresh n k)))
                 (≼-cong-∥ F (≼-refl ≈-refl))
...
Γ-S / p ⊢-≋ refl     = p
Γ-S / p ⊢-≋ (x ◅ xs) = Γ-S / go Γ-S x p ⊢-≋ xs where
  ...
  go {Γ = Γ} {γ = γ} Γ-S (fwd (ν-ext′ {B₁ = B₁} {B₂})) p₀
    ...
    = let
      ...
      ≤ = begin
          ...
          ≲⟨ ≼-cong-∥ (≼-refl refl) (𝐂.≼-⋯ (𝐂.⇔→⇒ ⦃ 𝐂.Kᵣ ⦄ {Γ} (𝐂.wk*-⇔ ⦃ 𝐂.Kᵣ ⦄ (Γ₁ ⸴* Γ₂))) ≤γ) ⟩
            (structBinder B₁ 𝐂.⋯ᵣ 𝐂.wkʳ (sum B₂) 𝐂.⋯ᵣ 𝐂.wkʳ _)
              ∥ (structBinder B₂ 𝐂.⋯ᵣ 𝐂.wkˡ (sum B₁) 𝐂.⋯ᵣ 𝐂.wkʳ _)
              ∥ (γ 𝐂.⋯ᵣ 𝐂.weaken* (sum B₁ + sum B₂))
          ∎
  ...
  go Γ-S (fwd (ν-cong′ eq)) p₀
    ...
    = TP-Res N pol ⊢B₁ ⊢B₂ C C′ (go (All.++⁺ (All.++⁺ (bindCtx⇒chanCtx C) (bindCtx⇒chanCtx C′)) Γ-S) (fwd eq) p)
  go Γ-S (bwd ∥-comm′) p₀
    with _ , _ , γ≤ , p , q ← inv-∥ p₀
    = TP-Weaken (≼-respˡ-≈ 𝐂.∥-comm γ≤) (TP-Par q p)
  go Γ-S (bwd ∥-assoc′) p₀
  ...
  go {Γ = Γ₀} Γ-S (bwd (ν-ext′ {B₁ = B₁} {B₂ = B₂})) p₀
    ...
    with γ₁′ , H2 , p₁′ ← p₁ ⊢⋯ₚ⁻¹ ⊢weaken* (Γ₁ ⸴* Γ₂) _ / wk*-inj (sum B₁ + sum B₂)
    with ζ₂ , O1 , O2 ← νext-split (Γ₁ ⸴* Γ₂)
                          ([ dom⋯wkʳ⊆fresh _ (structBinder B₁ 𝐂.⋯ᵣ 𝐂.wkʳ (sum B₂))
                           , dom⋯wkʳ⊆fresh _ (structBinder B₂ 𝐂.⋯ᵣ 𝐂.wkˡ (sum B₁)) ]′ ∘ x∈p∪q⁻ _ _)
                          H1raw
                          (subst (_ ∶_≼ _) (sym (brₛ (⊢weaken* (Γ₁ ⸴* Γ₂) Γ₀) γ₁′)
  ...
  go Γ-S (bwd (ν-cong′ eq)) p₀
    with _ , _ , _ , pol , N , ⊢B₁ , ⊢B₂ , C , C′ , p ← inv-ν p₀
    = TP-Res N pol ⊢B₁ ⊢B₂ C C′ (go (All.++⁺ (All.++⁺ (bindCtx⇒chanCtx C) (bindCtx⇒chanCtx C′)) Γ-S) (bwd eq) p)
