...
_⊢⋯_ : ⦃ K : Kit 𝓕 ⦄ ⦃ W : WkKit K ⦄ ⦃ TK : TKit K ⦄ →
       ...
       {ϕ : m –[ K ]→ n} {σ : _} →
       Γ₁ ; γ ⊢ e ∶ T ∣ ϵ →
       ϕ ∶ σ ⊢[ TK ] Γ₁ ⇒ Γ₂ × σ 𝐂.Preserves[ Mobile ] Γ₁ ⇒ Γ₂ × σ 𝐂.Preserves[ Unr ] Γ₁ ⇒ Γ₂ →
       Γ₂ ; γ 𝐂.⋯ σ ⊢ e ⋯ ϕ ∶ T ∣ ϵ
T-Const x ⊢⋯ ⊢ϕ = T-Const x
_⊢⋯_ {γ = γ} {σ = σ} (T-Var x T-eq) (⊢ϕ , _ , _) =
  let open ≈-Reasoning in
  ⊢`/id (subst 𝓕[ _ ; _ ⊢ _ ∶_] T-eq (⊢ϕ x))
_⊢⋯_ {γ = γ} (T-Abs {d = d} 𝓂→C x) (⊢ϕ , σ-mob , σ-unr) =
  let open Fin.Patterns in
  let eq = join-⋯ d (` 0F) _
             ■ cong (join d (` 0F)) (sym (𝐂.⋯-↑-wk γ _))
  in
  T-Abs (𝐂.allCx-⋯ σ-mob ∘ 𝓂→C)
    $ subst-γ eq
    $ x ⊢⋯ (⊢↑ ⊢ϕ , 𝐂.↑-preserves σ-mob , 𝐂.↑-preserves σ-unr)
T-App {d = d} {γ₁} {γ₂} e₁ e₂ ⊢⋯ ϕ∶σ =
  subst-γ (sym (join-⋯ d γ₂ γ₁)) $
    T-App (e₁ ⊢⋯ ϕ∶σ) (e₂ ⊢⋯ ϕ∶σ)
T-Pair p/s {γ₁} {γ₂} x₁ x₂ seq→ℙ ⊢⋯ ϕ∶σ =
  subst-γ (sym (join-⋯ p/s γ₁ γ₂)) $
    T-Pair p/s (x₁ ⊢⋯ ϕ∶σ) (x₂ ⊢⋯ ϕ∶σ) seq→ℙ
T-Let p/s {γ₁} {γ₂} x₁ x₂ ⊢⋯ ϕ∶σ@(⊢ϕ , σ-mob , σ-unr) =
  let open Fin.Patterns in
  ...
  subst-γ (sym (join-⋯ p/s γ₁ γ₂))
    ...
    $ subst-γ eq
    $ x₂ ⊢⋯ (⊢↑ ⊢ϕ , 𝐂.↑-preserves σ-mob , 𝐂.↑-preserves σ-unr)
T-LetUnit p/s {γ₁} {γ₂} x x₁ ⊢⋯ ϕ∶σ =
  subst-γ (sym (join-⋯ p/s γ₁ γ₂)) $
    T-LetUnit p/s (x ⊢⋯ ϕ∶σ) (x₁ ⊢⋯ ϕ∶σ)
T-LetPair {d = d} p/s {γ₁} {γ₂} x x₁ ⊢⋯ ϕ∶σ@(⊢ϕ , σ-mob , σ-unr)  =
  let open Fin.Patterns in
  ...
  subst-γ (sym (join-⋯ p/s γ₁ γ₂))
    $ T-LetPair p/s (x ⊢⋯ ϕ∶σ)
    $ subst-γ eq
    $ x₁ ⊢⋯ (⊢↑ (⊢↑ ⊢ϕ) , 𝐂.↑-preserves (𝐂.↑-preserves σ-mob) , 𝐂.↑-preserves (𝐂.↑-preserves (σ-unr)))
T-Weaken ϵ≤ γ≤ x ⊢⋯ ϕ∶σ@(⊢ϕ , σ-mob , σ-unr) =
  T-Weaken ϵ≤ (𝐂.≼-⋯ σ-unr γ≤) $
    x ⊢⋯ (⊢ϕ , σ-mob , σ-unr)
...
instance
  ...
  TKₛ = record
    ...
    ; ⊢id/` = λ x → T-Var _ refl
    ; ⊢`/id = λ x → x
    ; ⊢wk   = λ {_} {Γ} {γ} {T} x → subst (_ ;_⊢ _ ∶ _ ∣ _) (𝐂.weaken/wk γ) $
                x ⊢⋯ (⊢weaken ⦃ TKᵣ ⦄ {T = T} Γ , (λ _ → `_) , (λ _ → `_))
    }
...
_⊢⋯ₛ_ : {ϕ : m →ₖ n} {σ : _} →
        Γ₁ ; γ ⊢ e ∶ T ∣ ϵ →
        ϕ ∶ σ ⊢[ TKₛ ] Γ₁ ⇒ Γ₂ × σ 𝐂.Preserves[ Mobile ] Γ₁ ⇒ Γ₂ × σ 𝐂.Preserves[ Unr ] Γ₁ ⇒ Γ₂ →
        Γ₂ ; γ 𝐂.⋯ σ ⊢ e ⋯ ϕ ∶ T ∣ ϵ
_⊢⋯ₛ_ = _⊢⋯_ ⦃ TK = TKₛ ⦄
